...
```{r, fig.alt="Time series of Apple stock prices from 2000 until 2020. Strong upwards trend from 0.85 USD to 1359 USD.", fig.cap = "Apple stock prices. Prices are in USD, adjusted for divident payments and stock splits. Source: Yahoo!Finance"}
prices |>
...
```{r, fig.cap = "DOW index stock prices. Prices in USD, adjusted for dividend payments and stock splits.", fig.alt = "Many time series with daily prices. General trend positive for the stocks in the DOW index."}
index_prices |>
  ggplot(aes(
    x = date,
    y = adjusted,
...
```{r, fig.cap = "Aggregate daily trading volume of DOW constituents in billion USD.", fig.alt = "Time series with daily trading volume, ranging from 15 in 2000 to 60 in 2020."}
volume <- index_prices |>
...
```{r, fig.cap = "Trading volume on Dow Index versus previous day trading volume", fig.alt = "A scatterplot which shows that previous day aggregate trading volume and aggregate trading volume neatly line up along a 45 degree line. "}
volume |>
...
```{r, fig.cap = "Efficient frontier for Dow Jones constituents as asset universe", fig.alt = "Figure shows Dow Jones constituents in a mean-variance diagram. A hyperbola indicates the efficient frontier of portfolios that dominate the individual holdings in the sense that they deliver higher expected returns for the same level of volatility."}
res |>
