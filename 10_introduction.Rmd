...
The function `lag` computes the previous value in a vector. 
...
The resulting tibble contains three columns where the last contains the daily returns. 
Note that the first entry naturally contains `NA` because there is no previous price. 
...
Additionally, we also add a dashed red line that indicates the 5\% quantile of the daily returns to the histogram, which is a (crude) proxy for the worst return of the stock with a probability of at least 5\%. 
...
We see that the maximum *daily* return was around `r returns |> pull(ret) |> max() * 100` percent.  
...
The resulting file contains `r nrow(index_prices)` daily observations for in total `r index_prices |> count(company) |> nrow()` different corporations. 
