...
Next, we load the necessary stock market (CRSP) and accounting (Compustat) data from my SQLite database. As indicated above, the data is already prepared following the corresponding chapter in Tidy Finance. However, you can also review the code for this part in the following [Gist](https://gist.github.com/patrick-weiss/08c4bb0c6b31028921faa5ca6b510abc).
...
crsp_monthly <- msf_db |>
  ...
  inner_join(
    msenames_db |>
      ...
      select(permno, ncusip, exchcd, siccd, namedt, nameendt),
    by = c("permno")
  ) |>
  ...
  select(
    permno, cusip, ncusip, month, ret, retx,
    shrout, altprc, exchcd, siccd, dlret, dlstcd
  ) |>
...
highlight <- tibble(
  x = density(data_premia$premium_estimate)$x,
...
nse_thumbnail <- data_premia |>
  ...
  annotate("text",
    x = 1.1, y = 1.7, label = "Non-Standard Errors",
    color = "#E1AF00", size = 10
  ) +
  annotate("text",
    x = 1.25, y = 1.35, label = "In Portfolio Sorts",
    color = "#E1AF00", size = 10
  )
