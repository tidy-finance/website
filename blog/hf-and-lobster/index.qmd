...
image: logo.png
image-alt: "Logo of the lobsteR R package for downloading and cleaning LOBSTER high-frequency order book data"
execute: 
  warning: false
---
...
This is part one of two blog posts in which we announce and explain these two major updates to the [tidy-finance.org](https://tidy-finance.org) ecosystem.
...
After cleaning, more than 11 million observations remain for this single trading day. To visualize the flash crash, I aggregate to a 1-second grid: each message is assigned to a `ts_interval` and summary statistics — midquote, spread, depth, and volume — are computed per interval, time-weighted by the duration each state persisted (`ts_latency`).
...
orderbook_aggregated |>
  filter(ts_interval>=as.POSIXct(glue::glue("{date} 14:35:00"), tz = "UTC"),
ts_interval<=as.POSIXct(glue::glue("{date} 15:00:00"), tz = "UTC")) |>
  as_tibble() |>
  mutate(across(contains("depth5"), ~.x * opening_price / 1000000)) |>
  select(ts = ts_interval, `Midquote (USD)` = midquote, `Spread (bp)` = spread, `Depth bid (mUSD)` = depth5_bid, `Depth ask (mUSD)` = depth5_ask) |>
  pivot_longer(-ts) |>
...
The cleaning pipeline above transforms raw LOBSTER files into a compact, analysis-ready dataset. In the [companion blog post](../hf-and-hugging-face/index.qmd), we describe how we applied this pipeline to the full 2007--2025 LOBSTER history and made the resulting 5-second aggregated data for SPY freely available via `tidyfinance::download_data("hf")`.
