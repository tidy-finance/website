...
Note that the packages `glmnet`, `ranger`, and `brulee` are only needed if you want to implement the elastic net, random forests, or neural networks, respectively. 
```{r}
library(tidyverse)
library(archive)
library(arrow)
...
I tune the model and - after selecting the optimal tuning parameter value based on the prediction error in the validation set - fit the penalized regression using the available data (training and validation test set). The process can be faster by distribution the parameter tuning across multiple workers. `tidymodels` takes care of this as long as the user specifies a plan. I set the option `parallel_over = "resamples"` so that the formula is processed only once and then reused across all models that need to be fit. Parallelisation across the hyperparameter grid in `tidymodels` is done by initiating a `plan()` according to the [documentation](https://tune.tidymodels.org/articles/extras/optimizations.html).
...
```{r}
fitted_workflow <- butcher::butcher(ml_workflow_final)
readr::write_rds(fitted_workflow, file = "data/fitted_workflow.rds")
```
...
fitted_workflow <- readr::read_rds("data/fitted_workflow.rds")
oos_predictions <- fitted_workflow |> predict(oos)
...
hml_portfolio <- ml_portfolios |> 
    reframe(ret_excess = ret_excess[portfolio == 10] -
                  ret_excess[portfolio == 1],
...
ml_portfolios |>
    ...
    summarise(Pred = mean(ret_predicted), 
              Avg = mean(ret_excess),
              SD = sd(ret_excess),
SR = Avg / SD) |>
print(n=Inf)
...
## Some comments on parallelisation
