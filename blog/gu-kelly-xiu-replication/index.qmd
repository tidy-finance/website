...
date: "2025-06-01"
description: A partial replication in R of the paper *Empirical Asset Pricing via Machine Learning*.
image: thumbnail.jpeg
image-alt: This image visually compares machine learning methods in finance for measuring equity risk premiums. It features a neural network model, predictive charts, and Sharpe ratio comparisons. The color scheme conveys reliability and trustworthiness. Created with Copilot.
categories: 
  - R
  - Replications
  - Machine learning
...
library(tidymodels)
library(future)
# install.packages("glmnet") # In the application below, tidymodels requires glmnet so make sure to install the package before running the code
```
...
The idea is that at each date, the cross-section of each predictor should be scaled such that the maximum value is 1 and the minimum value is -1. The cross-sectional ranking is be time-consuming. The function below explicitly handles `NA` values, so they do not tamper with the ranking. 
...
To retrieve CRSP excess returns, we merge the stock-level characteristics with the prepared monthly CRSP data as described in [Tidy Finance with R](https://www.tidy-finance.org/r/wrds-crsp-and-compustat.html). To create portfolio sorts based on the predictions, `mktcap_lag` remains in the sample (albeit it should be mentioned that the main results in Section 2.4.2 of the paper are derived computing equal-weighted portfolios).
...
# TODO: check if macro predictors should be lagged in this application
...
The following recipe creates dummies for each `sic2` code and adds interaction terms between macro variables and stock characteristics. Note that `data` for the recipe is only required to identify the columns in the sample. No computations are performed at the first step. 
...
Next, I create the full dataset containing `{r} nrow(characteristics)` monthly observations and 920 covariates. 
```{r}
characteristics_prepared <- bake(rec, new_data = characteristics)
```

I store the prepared file for your convenience. Furthermore, the authors neer use the entire dataset for training, instead, only a fraction needs to be available in memory at any given point in time. 
...
We visualize our understanding of this data-budget below.
...
visualisation_data = pmap_dfr(estimation_periods, 
                              \(training_start, 
                                training_end, 
                                validation_start, 
                                validation_end, 
...
For the sake of simplicity, Ifit linear models using $l2$ loss functions. In `R`, the package `hqreg` implements algorithms for fitting regularization paths for Lasso or elastic net penalized regression models with Huber loss. However, `hqreg` is not a running engine for `pairsnip` (yet). 
...
For instance, to fit the elastic net it seems necessary to evaluate the mean-squared prediction error in the validation sample for *only* two values of the penalty term. Instead of tuning the elastic net mixture coefficient, I follow the paper and fix its value to $0.5$. 

In the following I rely on the well-documented `tidymodels` workflow to estimate the elastic net. 
...
The following performs the model estimation to create predictions for the year 1987. For a complete replication of the procedure laid out in the paper, you'll have to run a loop over all out-of-sample years (`i`).

```{r}
i <- 1
split_dates <- estimation_periods |> filter(row_number() == i)

train <- characteristics_prepared |> 
  filter(year(month) < split_dates$training_end) |> 
  collect()
validation <- characteristics_prepared |>  
  filter(year(month) >= split_dates$validation_start & year(month) <= split_dates$validation_end) |>
  collect()
...
show_best(ml_fit, metric = "rmse")
```
Above we see the mean-squared prediction error for the validation period (`{r} split_dates$validation_start` until `{r} split_dates$validation_end`) for the two different values for the penalty parameter in the elastic net. If more computation time is available, it is likely advisable to expand the number of evaluation by providing a wider grid of possible parameter constellations. 
...
While this blog post only shows how to implement the prediction procedure based on the elastic net, it is no problem to generate predictions for the remaining main methods (random forests, neural networks) presented in the paper. Below, I provide code for models that come as close as possible to the specifications laid out in the Online Appendix of the paper. 

For the random forest, Table A5 in the Online Appendix states that the number of grown trees is fixed to 300. The number of features in each split is set to $\in \{3, 5, 10, 20, 30, 50, ...\}$. It is not clear to me what $...$ means in that context, for that reason I fix the number of choices to six. Finally, in the paper *depth* is chosen asa hyperparameter, ranging from one (pruned tree) to six (more complex tree). `ranger` does not support *depth* as a hyperparameter but rather allows to dynamically stop growing further branches once a minimum number of observations in a group is reached. The effect is similar (if `min_n` is small, the tree can grow deep, otherwise we expect pruned trees), but cannot be translated directly into `depth`. 
...
The following code chunks performs the predictions for the out-of-sample year (`{r} split_dates$oos_year`): First, I predict stock excess returns based on the fitted model, then we sort assets into portfolios based on the prediction. I chose a value-weighted scheme which is arguably less sensitive to trading cost (see the discussion on p. 2265 of the paper).

```{r}
oos <- characteristics_prepared |> 
  filter(year(month) == split_dates$oos_year) |>
  collect()

oos_predictions <- ml_workflow_final |> predict(oos)

oos <- oos |>
  select(permno, month, mktcap_lag, ret_excess) |>
  bind_cols(oos_predictions)
```
...
TODO: evaluation in terms of Predicted return Average return SD SR for the 10 portfolios + 10-1
