...
The output of `evaluate()` already provides evaluation metrics in a structured way. We simply have to average the metrics over the cross-validation folds.
...
For each recommender, we are interested the following numbers:
...
# Create Predictions for New Users
...
So the IBCF algorithm recommends XXX as a stock if you are already invested in Nvidia and Apple. Of course, this recommendation is not serious investment advice, but rather serves an illustrative purpose. 