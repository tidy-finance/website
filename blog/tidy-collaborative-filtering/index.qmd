...
Hybrid recommender systems combine both collaborative and content-based filtering to overcome the challenges of each approach. There are different hybridization techniques available, e.g., combining the scores of different components (weighted), choosing among different component (switching), following strict priority rules (cascading), presenting outputs from different components at the same time (mixed), etc.

# Train Collaborative Filtering Recommenders in R

For this post, we rely on the `tidyverse` [@Wickham2019] family of packages, `scales` [@scales] for scale functions for visualization, and `recommenderlab` [@recommenderlab] - a package that provides an infrastructure to develop and test collaborative filtering recommender algorithms.  
...
I load a data set with stock holdings of investable wikifolios at the beginning of 2023. The data contains the portfolios of 6,544 wikifolios that held in total 5,069 stocks on January 1st 2023. Feel free to reach out to me if you want to get access to the data. 
