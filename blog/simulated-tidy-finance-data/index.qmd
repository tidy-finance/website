...
tbl(tidy_finance_simulated, "crsp_daily")
```
...
mergent_simulated <- 1:number_of_bonds |> 
  map_df(
    function(x) {
      tibble(
...
dbWriteTable(tidy_finance_simulated, "mergent", 
             mergent_simulated, overwrite = TRUE)
...
trace_enhanced_simulated <- bind_rows(
...
dbWriteTable(tidy_finance_simulated, "trace_enhanced", 
             trace_enhanced_simulated, overwrite = TRUE)
```

```{r}
dbSendQuery(tidy_finance_simulated, "VACUUM;")
```
