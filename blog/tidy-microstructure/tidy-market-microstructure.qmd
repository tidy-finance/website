...
<!-- TODO: activate link. We'll change it to github. -->
...
<!-- Niklas: Is already in place by the trade and quote data inspection. As such, changing it would lead to a whole different structure which is not wanted.--> 
...
<!-- Niklas: Don't change the RIC to ticker and venue in a single venue setting as the code is not worth it.-->
...
Let's dive into it. After loading the required packages, the following code shows how to import and preview the data. We get an overview of the data by simply typing the name of the `data.table` in the console. It automatically abbreviates the content to show only the first and last five rows of data. 
...
We have two timestamps for each observation. The `exch_time` variable is assigned by the exchange at the time an event is recorded in the exchange matching engine. The `date_time` variable is the timestamp assigned on receipt at the data vendor, which is by definition later than the `exch_time`. Exchanges that are located at different distances from the vendor are likely to have different reporting delays. It is then up to the researcher to determine which timestamp to rely on, and the choice may depend on the research question. In our setting, as we measure liquidity across venues, it is important that the time stamps across venues are comparable. Based on that each exchanges has strong incentives to assign accurate time stamps (to cater for low-latency participants), we choose to work with the `exch_time` variable.  
...
<!-- TODO: specifically define P^A and P^B: DONE --> 
...
```
## tidyverse (using dtplyr)
<!-- The code below overwrites the tv_quotes as such I would recommend the second code -->
```{r}
...
```
:::
R produces a warning when plotting the nominal bid-ask spread. It mentions 1,576 rows containing "non-finite values". The non-finite values refer to `NA`, `Inf` (inifinite) and `-Inf` (negative inifinite). In the timestamp section, we imposed `NA` for LOB variables during midday auction. To see if those are the cause of the warning, let's create a histogram of the time stamps of the missing values. 
...
<!-- **Data export.** -->
<!-- As the quote data processing is often computationally expensive, it is useful to save a clean version of the `quotes` to disk.  -->

<!-- ::: {.panel-tabset group="language"} -->
<!-- ## data.table -->
<!-- ```{r} -->
<!-- # Export quotes -->
<!-- fwrite(quotes, file = "temp_quotes.csv.gz") -->
<!-- ``` -->
...
<!-- TODO: Suggest to remove the required data folder structure for the reader such that the code can run seamlessly. OK-->
<!-- TODO: Suggest to use a generally readable file format such as feather or a zipped csv (.csv.gz) DONE -->
...
<!-- TODO: could be useful to define tick_size here. Done Check for Bjorn. Björn: The tick size is defined earlier. -->
<!-- TODO: I suppose the tick size is half a cent at BOTH venues? Done Check for Bjorn. Yes, it is always the same across EU venues. -->
...
```
:::
<!-- TODO: I suggest to scale by 1e4 and 1e-5 in the first step and to round all columns at once in the second step Done -->
...
<!-- TODO: confused about the "negative sign". Bjorn -- done -->
...
<!-- TODO: I guess you mean relative spreads wider than 500 basis points? Bjorn - Done -->
...
```
:::
<!-- TODO: consider multiplying by 100 for interpretation in percent Done -->
...
```
:::
<!-- TODO: I advice to put this part to the beginning as well.  Decided against it-->
<!-- TODO: Also here I would clean the column names as a first thing while quickly explaining what they mean. Drop the ones you do not need.  Decided against it-->
...
::: {.panel-tabset group="language"}
## data.table
<!--Niklas: Bjorn, we can think of adding the facet_warp command to be accurate on not showing both price series as one. 
Björn: Good idea, but I decided to use it for dates instead :). As we find out below that the problem comes from the LSE trades, there is also no need to show that here. --> 
```{r Trade price plot}
...
<!-- Niklas: Bjorn maybe we write a part here to explain that the outliers are predominately caused by dark trades on LSE.
Björn: I think the text below explains that.-->
...
<!-- Niklas: I included the blog post of R bloggers on it -->
...
<!-- TODO: I suggest you stick to either TQE or Turquoise through the entire document after defining it once. Done-->
...
<!-- Niklas: Bjorn We are often reading the ebbo because we are overwriting it, I changed that. 
Björn: I changed it back. Sorry, didn't like it. Is it not better to just retain the true time stamp? Here for the efficiency measures, I think we should simply reload it. -->
::: {.panel-tabset group="language"}
## data.table
```{r Load EBBO data}
quotes_ebbo <- fread(file = "quotes_ebbo.csv.gz")
...
```{r Convert to RScript, include= FALSE, eval= FALSE}
