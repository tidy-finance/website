...
```{r}
tv_quotes_ebbo <- tv_quotes_ebbo |>
  mutate(quote_time = time + 0.000001) |>
  select(-time) |>
  arrange(date, quote_time) |> 
  as_tibble()
...
tv_trades <- tv_trades |>
  left_join(tv_quotes_ebbo, join_by(date, closest(time >= quote_time))) |>
  lazy_dt()
```
:::
...
tv_trades_liquidity <- tv_trades |>
  ...
  group_by(ticker, date) |>
  summarise(
    midpoint_w = weighted.mean((best_bid_price * best_ask_depth + best_ask_price * best_bid_depth) /
                                (best_ask_depth + best_bid_depth), w = size),
    effective_spread = weighted.mean(2 * dir * (price - midpoint) / midpoint, w = size),
...
```{r}
tv_quotes_ebbo <- tv_quotes_ebbo |>
  mutate(time = quote_time - 60)

# Rename variables to indicate that they correspond to quotes 1 minute after the trade
# The function `paste0` adds the suffix "_1min" to each variable
tv_quotes_ebbo <- tv_quotes_ebbo |> 
  rename_with(~str_c(., "_1min"), c(midpoint, crossed, locked, large))

# Merge trades and quotes
tv_trades <- tv_quotes_ebbo |>
  left_join(tv_trades, join_by(date, closest(time>=time))) |>
  arrange(ticker, date, time.x) |>
  group_by(ticker, date) |>
...
tv_quotes_1sec <- tv_quotes_1sec |> 
  left_join(tv_quotes_ebbo, join_by(date, closest(time>=time))) |>
  mutate(midpoint_1min = na_if(midpoint_1min, locked_1min|crossed_1min|large_1min))
