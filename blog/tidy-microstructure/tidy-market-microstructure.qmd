...
## tidyverse (using dtplyr)
```{r}
as_tibble(tv_quotes) |> count(ticker, type, domain)
tv_quotes <- tv_quotes |>
  select(-type, -domain)
```
...
as_tibble(tv_quotes) |> count(ticker, date) |> pivot_wider(names_from = ticker, values_from = n)
```
:::
...
```{r}
tv_quotes <- tv_quotes |>
  mutate(time = hms::as_hms(exchange_time),
         time = hms::as_hms(time + dhours(gmt_offset)))
```

Having made sure that dates and times are in the desired format, we can save space by dropping the raw time and date variables. 

```{r}
tv_quotes <- tv_quotes |> select(-date_time, -exchange_time, -gmt_offset)
```
:::
...
First, we exclude quotes around the opening and closing of continuous trading.
Then, we set quotes around the intraday auction to missing. 
```{r}
tv_quotes <- tv_quotes |>
