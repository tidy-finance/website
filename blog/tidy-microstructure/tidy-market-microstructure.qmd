...
```{r}
#| eval: FALSE
write_csv(as_tibble(tv_quotes_ebbo), file = "quotes_ebbo.csv.gz")
...
# Merge trades and trade price changes
tv_trades <- tv_trades %>%
  left_join(price_tick, by = c("date", "time")) %>%
  arrange(date, time) %>%
...
trades <- trades %>%
  ...
  mutate(
    dir = case_when(
      !is.na(quote_diff) & quote_diff != 0 ~ quote_diff,
      TRUE ~ price_change
    )
  ) %>%
  ungroup()

# Display table of ticker and dir
table_result <- table(trades$ticker, trades$dir)
print(table_result)
