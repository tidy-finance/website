...
In 2022, CRSP rolled out the new format of its data which is called **CRSP's Stock and Indexes Flat File Format 2.0 (CIZ)**. This new format is now also available via WRDS. So, what is new in the second version? The most notable items from CRSP's announcement includes:
...
- They introduce issuer level data, which involved ensuring unique observations for each issuer (e.g., industry codes).
...
You can access the new data via WRDS using the commands below. For the main data WRDS basically just added a "_v2" postfix. However, you also see that we need fewer tables, as we do not load information on delisting returns separately. 
...
To take a look at the new table and compare it to the old, we use the new `tidyfinance` R package to download the data. If you do not have the package yet, please install it from CRAN using `install.packages("tidyfinance")`. For ease of use, we also load the full `tidyverse` as in all our chapters of the R book.
...
Then, we can download the two data formats with the function `download_data_wrds_crsp`.

```{r}
#| warning: false
#| message: false

crsp_old <- download_data_wrds_crsp("crsp_monthly", 
                                    version = "v1",
                                    start_date = "1970-01-01",
                                    end_date = "2022-12-01")

crsp_new <- download_data_wrds_crsp("crsp_monthly", 
                                    version = "v2",
                                    start_date = "1970-01-01",
                                    end_date = "2022-12-01")
```
...
anti_join(crsp_new, crsp_old,
          by = join_by("permno", "date")) |> 
  nrow()
```
We find that roughly 4,000 observations are not matched, where the new data contains more rows. We are not sure where these observations come from or where we lost them in the old data. However, this is a very minor difference. 

Next, we check if the monthly returns are all the same. In fact, they should not be as CRSP states that they have updated the way to compound distributions. They also state this change in compounding as an example among other things. However, they do not tell us about these *other* things.

```{r}
inner_join(crsp_new |> 
             select(permno, date, ret_new = ret), 
           crsp_old |> 
             select(permno, date, ret_old = ret),
          by = join_by("permno", "date")) |> 
  mutate(differences = near(ret_new, ret_old, tol = 10^-6)) |> 
  summarize(differences = all(differences, na.rm = TRUE))
```
...
Unfortunately, the old format (also called **1.0 (SIZ)**) will not receive updates after the end of the year 2024. Nevertheless, WRDS said that the old data will remain in its current place, which means we can continue replicating studies with their original data (ignoring changes to the actual data points). We have also included the option to download the legacy data in our `tidyfinance` R package.