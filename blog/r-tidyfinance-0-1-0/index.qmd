...
```
Under the hood, the function uses the `frenchdata` package and applies some cleaning steps, as in TFWR. If you haven't installed `frenchdata` yet, you'll get prompted to install it first before you can download this specific data type. 
...
However, if you want to add further columns, you can add them via `...` to `download_data_wrds_crsp()`, for instance:
...
Similarly, if you want to download the old version of CRSP, you can use the `version` parameter in `download_data_wrds_crsp()` (see this blog post here TODO: INSERT LINK). 
...
We include functions to check out content from tidy-finance.org. If you want to list all available R chapters, simply call the following function:
...
This opens a specific website or the main index page in you default web broswer. 
...
If you rather want to throw away the bottom and top quantiles of your distribution, then you can use `trim()`

```{r}
data |> 
  mutate(x_winsorized = trim(x, 0.01))
```
