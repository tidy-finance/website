...
The above function takes stock symbols as inputs (i.e., stock-specific identifiers). To get symbols you have now two options: first, we provide the symbols of all listed stocks at AMEX, NASDASQ and NYSE via the [Nasdaq Stock Screener](https://www.nasdaq.com/market-activity/stocks/screener). You can find details again using `?download_data_symbols`.

```{r}
download_data("symbols", exchange = "NASDAQ")
```

What you get is a data frame with stock symbols, corresponding company name, country of incorporation, the year of initial public offering (IPO), industry, and sector. 
