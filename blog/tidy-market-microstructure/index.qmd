...
```{r}
#| eval: false
write_csv(as_tibble(tv_quotes_ebbo), file = "tv_quotes_ebbo.csv.gz")
```
:::
...
trades <- quotes_ebbo[trades, on = c("date", matching_time = "time"), roll = TRUE, mult = "last"]
```
...
```{r}
#| eval: false
tv_quotes_ebbo <- read_csv("tv_quotes_ebbo.csv.gz")
```

```{r}
tv_quotes_ebbo <- tv_quotes_ebbo |>
  mutate(matching_time = time + 0.000001) |>
  arrange(date, matching_time) |> 
  as_tibble()
...
tv_trades <- tv_trades |>
  left_join(tv_quotes_ebbo |> select(-time), join_by(date, closest(time >= matching_time))) |>
  lazy_dt()
```
:::
...
tv_trades <- tv_trades |>
  as_tibble() |>
  left_join(price_tick, join_by(date, closest(time>=time))) |>
  rename(time=time.x) |>
...
quotes_ebbo <- quotes_ebbo[, shifted_time := matching_time - 60]
...
setkeyv(quotes_ebbo, cols = c("date", "shifted_time"))
setkeyv(trades, cols = c("date", "time"))
...
```{r}
tv_quotes_ebbo <- tv_quotes_ebbo |>
  mutate(shifted_time = matching_time - 60)
...
tv_trades <- tv_trades |>
  as_tibble() |>
  left_join(tv_quotes_ebbo |> select(date, shifted_time, contains("_1min")), 
            join_by(date, closest(time>=shifted_time))) |>
  arrange(ticker, date, shifted_time) |>
  group_by(ticker, date) |>
...
tv_quotes_1sec <- tv_quotes_1sec |> 
  left_join(tv_quotes_ebbo |> as_tibble(), join_by(date, closest(time>=time))) |>
...
Hagströmer, B. and Landsberg, N. (2023). Tidy Market Microstructure. URL: [www.tidy-finance.org/blog/tidy-microstructure](www.tidy-finance.org/blog/tidy-microstructure)
