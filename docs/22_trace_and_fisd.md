...
We start by looking into the number of bonds outstanding over time and compare it to the number of bonds traded in our sample. First, we compute the number of bonds outstanding for each quarter from 1990 until 2018. 
...
Next, we look at the bonds traded each quarter in our shorter period around the Paris Agreement from 2014 to 2016. Notice that we load the complete trace database of our sample, as we only have a single part of it in the environment from the download.
...
<img src="22_trace_and_fisd_files/figure-html/fig221-1.png" alt="Title:Number of bonds outstanding and traded each quarter. The figure shows a time series of outstanding bonds and bonds traded. The amount outstanding increases until 2000, when it starts decreasing. From 2010 onwards, it increases monotonically until the sample's end. On the other hand, the number of bonds traded from 2014 to 2018 represents only a fraction of roughly 60%, which does not change dramatically over the two years." width="90%" />
<p class="caption">(\#fig:fig221)The number of corporate bonds outstanding each quarter is from 1990 until 2018, as reported by Mergent FISD. The number of traded bonds from enhanced TRACE per quarter is from 2014 until 2016.</p>
</div>

We see that the number of bonds outstanding varies over time, with a peak before the turn of the millennium. After 2010, the number steadily increased, eventually surpassing the previous maximum. During our sample period of trade data, we see that the fraction of bonds trading each quarter is roughly 60%. On the flip side, the relatively small number of traded bonds means that many bonds do not trade through an entire quarter. This lack of trading activity illustrates the generally low level of liquidity in the corporate bond market, where it can be hard to trade specific bonds. 
