...
We recommend [Positron](https://positron.posit.co/) for all Tidy Finance work. Positron is a data-science environment from Posit that supports R and Python in a single application, with an integrated editor, console, terminal, and data viewer, as well as first-class support for Quarto documents. Download and install it from the [official website](https://positron.posit.co/).

Positron runs your code but does not bundle the languages themselves:

- R users need an R installation from [CRAN](https://cran.r-project.org/). Follow the official installation instructions for your operating system; there is nothing Tidy Finance-specific about this step.
- Python users do not need to install anything upfront. When you click *Start Session* in Positron and no suitable Python installation is available, Positron offers to install Python via [uv](https://docs.astral.sh/uv/), a fast Python package and project manager, and takes care of the setup for you (see the [Positron blog](https://opensource.posit.co/blog/2026-07-08_positron-uv/) for details).
