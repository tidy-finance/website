...
It is common to adjust for autocorrelation when reporting standard errors of risk premiums. As in [Univariate Portfolio Sorts](../chapters/univariate-portfolio-sorts.llms.md), the typical procedure for this is computing Newey and West ([1987](#ref-Newey1987)) standard errors.

## R

We again recommend the data-driven approach of Newey and West ([1994](#ref-Newey1994)) using the `NeweyWest()` function, but note that you can enforce the typical 6 lag settings via `NeweyWest(., lag = 6, prewhite = FALSE)`.
...
We compute the Newey-West \\t\\-statistics by fitting a constant-only model for each factor with a HAC covariance estimator using six lags.
...
- The `tidyfinance` package provides a user-friendly `estimate_fama_macbeth()` function that simplifies the Fama-MacBeth estimation pipeline.
