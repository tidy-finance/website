...
Building on our analysis from the previous chapter on [Modern Portfolio Theory](../chapters/modern-portfolio-theory.llms.md), we again examine the Dow Jones Industrial Average constituents as an exemplary asset universe. We download and prepare our monthly return data:
...
efficient_portfolios = pl.DataFrame(
    [
        {"symbol": r"\omega_{tan}", "mu": mu_w, "sigma": sigma_w},
...
[![Title: The efficient frontier with a risk-free asset and Dow index constituents. The figure shows a scatter plot with volatities on the horizontal axis and expected returns on the vertical axis.](capital-asset-pricing-model_files/figure-html/capm-fig-300-py-1.png)](capital-asset-pricing-model_files/figure-html/capm-fig-300-py-1.png "Expected returns and volatilities based on monthly returns adjusted for dividend payments and stock splits.")
...
[![](capital-asset-pricing-model_files/figure-html/capital-asset-pricing-model-security-market-line-3.png)](capital-asset-pricing-model_files/figure-html/capital-asset-pricing-model-security-market-line-3.png)
...
[![](capital-asset-pricing-model_files/figure-html/capital-asset-pricing-model-security-market-line-py-1.png)](capital-asset-pricing-model_files/figure-html/capital-asset-pricing-model-security-market-line-py-1.png)
...
alphas_figure = (
    ...
    + coord_flip()
    + labs(
        x="", y="Estimated asset alphas", fill="Significant at 95%?",
        title="Estimated CAPM alphas for Dow index constituents"
        )
)
alphas_figure.show()
...
[![Title: Estimated CAPM alphas for Dow index constituents. The figure shows a bar chart with estimated alphas and indicates whether an estimate is statistically significant at 95%. Only Nvidia exhibits a statistically significant positive alpha.](capital-asset-pricing-model_files/figure-html/capm-fig-305-py-1.png)](capital-asset-pricing-model_files/figure-html/capm-fig-305-py-1.png "Estimates are based on returns adjusted for dividend payments and stock splits and using the Fama-French market excess returns as a measure for the market.")

Estimates are based on returns adjusted for dividend payments and stock splits and using the Fama-French market excess returns as a measure for the market.
...
- The SMB (Small Minus Big) factor captures the tendency of small stocks to outperform large stocks, as we discuss in our chapter [Size Sorts and P-Hacking](../chapters/size-sorts-and-p-hacking.llms.md).
- The HML (High Minus Low) factor captures the tendency of value stocks to outperform growth stocks, as we show in [Value and Bivariate Sorts](../chapters/value-and-bivariate-sorts.llms.md).

Building on this framework, the Fama-French five-factor model ([Fama and French 2015](#ref-FamaFrench2015)) adds two more dimensions, which we later discuss in [Replicating Fama-French Factors](../chapters/replicating-fama-and-french-factors.llms.md):
...
[^1]: We prove in the Appendix [Proofs](../chapters/proofs.llms.md) that the efficient tangency portfolio \\\omega\_\text{tan}\\ can also be derived as a solution to the optimization problem \\\max\_{\omega} \frac{\omega^{\prime}\tilde\mu}{\sqrt{\omega^{\prime}\Sigma\omega}} \text{ s.t. } \omega^{\prime}\iota=1.\\
