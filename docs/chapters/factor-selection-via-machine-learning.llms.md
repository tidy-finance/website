...
Our data contains `{python} len(data.columns)-2` columns of regressors with the `{python} len(macro_predictors.columns) - 1` macro-variables and `{python} len(factors_ff3_monthly.columns) + len(factors_q_monthly.columns)-2` factor returns for each month. [Figure 1](#fig-1401) provides summary statistics for the `{python} industries_ff_monthly["industry"].n_unique()` monthly industry excess returns in percent. One can see that the dispersion in the excess returns varies widely across industries.
...
To start, we restrict our analysis to just one industry: Manufacturing. We split the sample into a *training* and a *test* set, holding out the last part of the data as a test set, which is not used for any model tuning. We use this test set to evaluate the predictive accuracy in an out-of-sample scenario. We define the cutoff date directly so that observations before it form the training set.
...
`scikit-learn` provides functionalities to construct a suitable grid of hyperparameters with `GridSearchCV()`. The code chunk below creates a \\10 \times 3\\ hyperparameters grid. Then, the method `fit()` evaluates all the models for each fold.
