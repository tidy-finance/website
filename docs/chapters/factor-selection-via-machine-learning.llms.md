...
The aim of this chapter is twofold. From a data science perspective, we introduce tools for modeling and machine learning (ML). From a finance perspective, we address the notion of *factor zoo* ([Cochrane 2011](#ref-Cochrane2011)) using ML methods. We introduce Lasso and Ridge regression as a special case of penalized regression models. Then, we explain the concept of cross-validation for model *tuning* with Elastic Net regularization as a popular example. We implement and showcase the entire cycle from model specification, training, and forecast evaluation. While the tools can generally be applied to an abundance of interesting asset pricing problems, we apply penalized regressions for identifying macroeconomic variables and asset pricing factors that help explain a cross-section of industry portfolios.
...
An alternative to Ridge regression is the Lasso (*l*east *a*bsolute *s*hrinkage and *s*election *o*perator). Similar to Ridge regression, the Lasso ([Tibshirani 1996](#ref-Tibshirani1996)) is a penalized and biased estimator. The main difference to Ridge regression is that Lasso does not only *shrink* coefficients but effectively selects variables by setting coefficients for *irrelevant* variables to zero. Lasso implements a \\L_1\\ penalization on the parameters such that: \\\hat\beta^\text{Lasso} = \arg\min\_\beta \left(Y - X\beta\right)'\left(Y - X\beta\right)\text{ s.t. } \sum\limits\_{k=1}^K\|\beta_k\| \< c(\lambda).\\ There is no closed-form solution for \\\hat\beta^\text{Lasso}\\ in the above maximization problem, but efficient algorithms exist (e.g., the `glmnet` package for R and Python). Like for Ridge regression, the hyperparameter \\\lambda\\ has to be specified beforehand.
...
To get started, we load the required packages and data.

## R

The main focus is on the workflow behind the `tidymodels` package collection ([Kuhn and Wickham 2020](#ref-tidymodels)), a collection of packages for modeling and ML using `tidyverse` principles. `tidymodels` comes with a handy workflow for all sorts of typical prediction tasks. Kuhn and Silge ([2018](#ref-Kuhn2022)) provide a thorough introduction into all `tidymodels` components. `glmnet` ([Simon et al. 2011](#ref-glmnet)) was developed and released in sync with Tibshirani ([1996](#ref-Tibshirani1996)) and provides an R implementation of Elastic Net estimation. The package `timetk` ([Dancho and Vaughan 2022](#ref-timetk)) provides useful tools for time series data wrangling.
...
The main focus is on the workflow behind the `scikit-learn` ([Pedregosa et al. 2011](#ref-scikit-learn)) package collection, a collection of packages for modeling and ML. `scikit-learn` comes with a handy workflow for all sorts of typical prediction tasks.
...
Our data contains 24 columns of regressors with the 13 macro-variables and 10 factor returns for each month. [Figure 1](#fig-1401) provides summary statistics for the 10 monthly industry excess returns in percent. One can see that the dispersion in the excess returns varies widely across industries.
...
[![Title: Excess return distributions by industry in percent. The figure shows boxplots that visualize the industry's excess return distribution. All industry returns are centered around zero and exhibit substantial outliers in the magnitude of 20 percent on a monthly basis.](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1401-py-1.png)](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1401-py-1.png "The box plots show the monthly dispersion of returns for 10 different industries.")

The box plots show the monthly dispersion of returns for 10 different industries.

## The Machine Learning Workflow

To illustrate penalized linear regressions, we employ a collection of packages for modeling and ML. Using the ideas of Ridge and Lasso regressions, the following example guides you through (i) pre-processing the data (data split and variable mutation), (ii) building models, (iii) fitting models, and (iv) tuning models to create the “best” possible predictions.
...
We employ the `scikit-learn` collection of packages for modeling and ML.

To start, we restrict our analysis to just one industry: Manufacturing. We split the sample into a *training* and a *test* set, holding out the last 20% of the data as a test set, which is not used for any model tuning. We use this test set to evaluate the predictive accuracy in an out-of-sample scenario.
...
Note that the resulting data contains the 137 observations from the test set and 154 columns. Why so many? Recall that the recipe states to compute every possible interaction term between the factors and predictors, which increases the dimension of the data matrix substantially.
...
We want to explain excess returns with all available predictors. The regression equation thus takes the form \\ r\_{t} = \alpha_0 + \left(\tilde f_t \otimes \tilde z_t\right)B + \varepsilon_t \\

where \\r_t\\ is the vector of industry excess returns at time \\t\\, \\\otimes\\ denotes the Kronecker product and \\\tilde f_t\\ and \\\tilde z_t\\ are the (standardized) vectors of factor portfolio returns and macroeconomic variables.
...
data = data.with_columns(interaction_terms)
...
That’s it - we are done! The object `lm_pipeline` contains the definition of our model with all required information, in particular the pre-processing steps and the regression model.
...
[![Title: Monthly realized and fitted manufacturing industry risk premium. The figure shows the time series of realized and predicted manufacturing industry risk premiums. The figure seems to indicate that the predictions capture most of the return dynamics.](factor-selection-via-machine-learning_files/figure-html/fig-1402-1.png)](factor-selection-via-machine-learning_files/figure-html/fig-1402-1.png "Figure 2: The figure shows monthly realized and fitted manufacturing industry risk premium. The grey area corresponds to the out of sample period.")
...
With the pipeline from above, we are ready to fit it to the data. Typically, we use training data to fit the model. The training data is pre-processed according to our pipeline steps, and the Lasso regression coefficients are computed. For illustrative purposes, we focus on the manufacturing industry for now.

``` python
data_manufacturing = data.filter(pl.col("industry") == "manuf").sort("date")

# Hold out the last 20% as a test set (matches R's initial_time_split(prop = 4/5)).
...
First, we focus on the in-sample predicted values \\\hat{y}\_t = x_t\hat\beta^\text{Lasso}.\\ [Figure 2](#fig-1402) illustrates the projections for the *entire* time series of the manufacturing industry portfolio returns. The grey area indicates the out-of-sample period, which we did not use to fit the model.
...
[![Title: Monthly realized and fitted manufacturing industry risk premium. The figure shows the time series of realized and predicted manufacturing industry risk premiums. The figure seems to indicate that the predictions capture most of the return dynamics.](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1402-py-1.png)](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1402-py-1.png "The figure shows monthly realized and fitted manufacturing industry risk premium. The grey area corresponds to the out of sample period.")

The figure shows monthly realized and fitted manufacturing industry risk premium. The grey area corresponds to the out of sample period.
...
To illustrate the difference between the `tidymodels` workflow and the underlying `glmnet` package, we compute the coefficients \\\hat\beta^\text{Lasso}\\ directly. Note that `glmnet` actually takes a vector `y` and the matrix of regressors \\X\\ as input. Moreover, `glmnet` requires choosing the penalty parameter \\\alpha\\, which corresponds to \\\rho\\ in the notation above. When using the `tidymodels` model API, such details do not need consideration.
...
[![Title: Estimated coefficient paths for different penalty factors. The figure shows how estimated lasso and ridge coefficients tend to zero for a higher penalty parameter. Ridge trace is smooth, and Lasso exhibits non-linear behavior.](factor-selection-via-machine-learning_files/figure-html/fig-1403-1.png)](factor-selection-via-machine-learning_files/figure-html/fig-1403-1.png "Figure 3: The figure shows estimated coefficient paths for different penalty factors. The penalty parameters are chosen iteratively to resemble the path from no penalization to a model that excludes all variables.")
...
coefficients_lasso = (
  pl.DataFrame(np.vstack(coefficients_lasso), orient="row")
  .with_columns(alpha=pl.Series(alphas), model=pl.lit("Lasso"))
  .unpivot(index=["alpha", "model"])
)
...
coefficients_ridge = (
  pl.DataFrame(np.vstack(coefficients_ridge), orient="row")
  .with_columns(alpha=pl.Series(alphas), model=pl.lit("Ridge"))
  .unpivot(index=["alpha", "model"])
)
```
...
coefficients_figure = (
  ggplot(
    pl.concat([coefficients_lasso, coefficients_ridge]),
...
[![Title: Estimated coefficient paths for different penalty factors. The figure shows how estimated lasso and ridge coefficients tend to zero for a higher penalty parameter. Ridge trace is smooth, and Lasso exhibits non-linear behavior.](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1403-py-1.png)](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1403-py-1.png "The figure shows estimated coefficient paths for different penalty factors. The penalty parameters are chosen iteratively to resemble the path from no penalization to a model that excludes all variables.")

The figure shows estimated coefficient paths for different penalty factors. The penalty parameters are chosen iteratively to resemble the path from no penalization to a model that excludes all variables.

### Tune a model

To compute \\\hat\beta\_\lambda^\text{Lasso}\\, we simply imposed a value for the penalty hyperparameter \\\lambda\\. Model tuning is the process of optimally selecting such hyperparameters through *cross-validation*. Again, we refer to any treatment of cross-validation to get a more detailed discussion of the statistical underpinnings. Here we focus on the general idea and the implementation.
...
For our sample, we consider a time-series cross-validation sample. This means that we tune our models with rolling-origin cross-validation, where each fold trains on a five-year window and validates on the subsequent, non-overlapping four-year window. For a grid of possible hyperparameters, we then fit the model for each fold and evaluate \\\hat{\text{MSPE}}\\ in the corresponding validation set. Finally, we select the model specification with the lowest MSPE in the validation set. First, we define the cross-validation folds based on our training data only.

``` r
# Shared rolling-origin folds: a five-year (60-month) training window and a
# subsequent, non-overlapping four-year (48-month) validation window. We build
# the folds from explicit row indices so they are identical to the Python ones.
make_rolling_folds <- function(df, train_len = 60, assess_len = 48, step = 48) {
  df <- arrange(df, date)
  n <- nrow(df)
  starts <- seq.int(0L, n - (train_len + assess_len), by = step)
  splits <- lapply(starts, function(s) {
    make_splits(
      list(analysis   = seq.int(s + 1L, s + train_len),
           assessment = seq.int(s + train_len + 1L, s + train_len + assess_len)),
      data = df
    )
  })
  manual_rset(splits, ids = paste0("Slice", seq_along(splits)))
}

data_folds <- make_rolling_folds(training(split))
data_folds
```
    ...
    # A tibble: 10 × 2
      splits          id    
      <list>          <chr> 
    1 <split [60/48]> Slice1
    2 <split [60/48]> Slice2
    3 <split [60/48]> Slice3
    4 <split [60/48]> Slice4
    5 <split [60/48]> Slice5
    # ℹ 5 more rows

Then, we evaluate the performance for a grid of different penalty values. `tidymodels` provides functionalities to construct a suitable grid of hyperparameters with `grid_regular`. The code chunk below creates a \\20 \times 3\\ hyperparameters grid. Then, the function `tune_grid()` evaluates all the models for each fold.
...
[![Title: Root MSPE for different penalty factors. The figure shows that more regularization does not affect the selected models in a meaningful fashion. At some point, the Elastic Net prediction error drops, which indicates the selected model. MSPE increases again for high penalization values.](factor-selection-via-machine-learning_files/figure-html/fig-1404-1.png)](factor-selection-via-machine-learning_files/figure-html/fig-1404-1.png "Figure 4: The figure shows root MSPE for different penalty factors. Evaluation of manufacturing excess returns for different penalty factors (lambda) and proportions of Lasso penalty (rho). 1.0 indicates Lasso, 0.5 indicates Elastic Net, and 0.0 indicates Ridge.")
...
For our sample, we consider a time-series cross-validation sample. This means that we tune our models with rolling-origin cross-validation, where each fold trains on a five-year window and validates on the subsequent, non-overlapping four-year window. For a grid of possible hyperparameters, we then fit the model for each fold and evaluate \\\hat{\text{MSPE}}\\ in the corresponding validation set. Finally, we select the model specification with the lowest MSPE in the validation set.

`scikit-learn` provides functionalities to construct a suitable grid of hyperparameters with `GridSearchCV()`. The code chunk below creates a \\100 \times 3\\ hyperparameters grid. Then, the method `fit()` evaluates all the models for each fold.

``` python
alphas = np.logspace(-6, 2, 100)

# Shared rolling-origin folds: a five-year (60-month) training window and a
# subsequent, non-overlapping four-year (48-month) validation window, built from
# explicit row indices so they are identical to the R folds.
def make_rolling_folds(n, train_len=60, assess_len=48, step=48):
  return [
    (np.arange(s, s + train_len),
     np.arange(s + train_len, s + train_len + assess_len))
    for s in range(0, n - (train_len + assess_len) + 1, step)
  ]

data_folds = make_rolling_folds(n_train)
...
validation = (
  pl.DataFrame({
    "param_regressor__alpha": np.asarray(
      finder.cv_results_["param_regressor__alpha"], dtype=float
    ),
    "param_regressor__l1_ratio": np.asarray(
      finder.cv_results_["param_regressor__l1_ratio"], dtype=str
    ),
    "mean_test_score": finder.cv_results_["mean_test_score"],
    "rank_test_score": finder.cv_results_["rank_test_score"]
  })
  .with_columns(mspe=-pl.col("mean_test_score"))
)

best_model = validation.sort("rank_test_score").head(1)
best_alpha = best_model["param_regressor__alpha"][0]
best_rho = float(best_model["param_regressor__l1_ratio"][0])
...
[![Title: Root MSPE for different penalty factors. The figure shows that more regularization does not affect the selected models in a meaningful fashion. At some point, the Elastic Net prediction error drops, which indicates the selected model. MSPE increases again for high penalization values.](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1404-py-1.png)](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1404-py-1.png "The figure shows root MSPE for different penalty factors. Evaluation of manufacturing excess returns for different penalty factors (lambda) and proportions of Lasso penalty (rho). 1.0 indicates Lasso, 0.5 indicates Elastic Net, and 0.0 indicates Ridge.")

The figure shows root MSPE for different penalty factors. Evaluation of manufacturing excess returns for different penalty factors (lambda) and proportions of Lasso penalty (rho). 1.0 indicates Lasso, 0.5 indicates Elastic Net, and 0.0 indicates Ridge.

[Figure 4](#fig-1404) shows that the MSPE drops faster for Lasso and Elastic Net compared to Ridge regressions as penalty factor increases. However, for higher penalty factors, the MSPE for Ridge regressions dips below the others, which both slightly increase again above a certain threshold. Recall that the larger the regularization, the more restricted the model becomes. The best performing model yields a penalty parameter (`alpha`) of 0.005 and a mixture factor (\\\rho\\) of 0.5.
...
select_variables <- function(input) {
  ...
  # Data folds for cross-validation (same rolling-origin scheme as above)
  data_folds <- make_rolling_folds(training(split))
...
``` python
all_industries = data["industry"].unique(maintain_order=True)

results = []
for industry in all_industries:
  print(industry)
  industry_training = (
    data.filter(pl.col("industry") == industry).sort("date").head(n_train)
  )
...
selected_factors = (
  pl.DataFrame({"variable": lm_pipeline[:-1].get_feature_names_out()})
  .with_columns(
    variable=pl.col("variable").str.replace_all("factor_|ff_|q_|macro_", "")
  )
  .with_columns([pl.Series(industry, result)
                 for industry, result in zip(all_industries, results)])
  .unpivot(index="variable", variable_name="industry", value_name="value")
  .filter(pl.col("value"))
)
```
...
[![Title: Selected variables for different industries. The figure shows which factors and macroeconomic predictors the Lasso model selected for the different industries. In general, there are not many selected variables. The market excess return is selected across all industries except for utilities.](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1405-py-1.png)](factor-selection-via-machine-learning_files/figure-html/factor-selection-via-machine-learning-fig-1405-py-1.png "The figure shows selected variables for different industries. Dark areas indicate that the estimated Lasso regression coefficient is not set to zero. White fields show which variables get assigned a value of exactly zero.")

The figure shows selected variables for different industries. Dark areas indicate that the estimated Lasso regression coefficient is not set to zero. White fields show which variables get assigned a value of exactly zero.

The heat map in [Figure 5](#fig-1405) conveys two main insights. First, we see a lot of white, which means that many factors, macroeconomic variables, and interaction terms are not relevant for explaining the cross-section of returns across the industry portfolios. In fact, only a couple of factors play a role for several industries. Second, there seems to be quite some heterogeneity across different industries. While barely any variable is selected by Lasso for Utilities, many factors are selected for, e.g., High-Tech and Durable, but they do not coincide at all. In other words, there seems to be a clear picture that we do not need many factors, but Lasso does not provide a factor that consistently provides pricing abilities across industries.

## Key Takeaways

- The `tidymodels` framework in R and the `scikit-learn` framework in Python enable a clean, modular workflow for pre-processing financial data, fitting models, and tuning hyperparameters through cross-validation.
