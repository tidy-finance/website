...
- Free Cash Flow (FCF) forecasts represent the expected future cash available for distribution to investors after accounting for operating expenses, taxes, and investments.
- Terminal value captures the company’s value beyond the explicit forecast period, often representing a significant portion of the total valuation.
- Discount rate, typically the Weighted Average Cost of Capital (WACC), adjusts future cash flows to present value by incorporating risk and capital structure considerations.
...
dcf_data = (income_statements
  .with_columns(
    ...
    ebit=pl.col("net_income") + pl.col("income_tax_expense") + pl.col("interest_expense") - pl.col("interest_income")
  )
  .select(["fiscal_year", "ebit", "revenue", "depreciation_and_amortization", "income_tax_expense"])
  .rename({
      "fiscal_year": "year",
  ...
  .join(
    (cash_flow_statements
      .select(["fiscal_year", "change_in_working_capital", "capital_expenditure"])
  ...
  .with_columns(
    fcf=pl.col("ebit") + pl.col("depreciation_and_amortization") - pl.col("taxes") + pl.col("delta_working_capital") - pl.col("capex")
  )
  .sort("year")
)
```
...
First, we express each FCF component as a ratio relative to revenue. This standardization helps identify trends and makes forecasting more systematic. [Figure 1](#fig-500) shows the historical evolution of these key financial ratios.
...
dcf_data = (dcf_data
  .with_columns(
    revenue_growth=pl.col("revenue") / pl.col("revenue").shift(1) - 1,
    operating_margin=pl.col("ebit") / pl.col("revenue"),
    da_margin=pl.col("depreciation_and_amortization") / pl.col("revenue"),
    taxes_to_revenue=pl.col("taxes") / pl.col("revenue"),
    delta_working_capital_to_revenue=pl.col("delta_working_capital") / pl.col("revenue"),
    capex_to_revenue=pl.col("capex") / pl.col("revenue")
  )
)
...
We demonstrate this forecasting approach in [Figure 2](#fig-501). Note that our forecast ratio dynamics just serve as an example and are not grounded in deeper economic analysis.
...
forecast_ratios = forecast_ratios.with_columns(type=pl.lit("Forecast"))

dcf_data = dcf_data.with_columns(type=pl.lit("Realized"))
dcf_data = pl.concat([dcf_data, forecast_ratios], how="diagonal")
...
dcf_data_long = dcf_data_long.with_columns(
  type=pl.col("type").cast(pl.Enum(["Realized", "Forecast"]))
)
...
dcf_data = (dcf_data
  .join(gdp_growth, on="year", how="left")
)
```
...
dcf_data = (dcf_data
  .with_columns(
    revenue_growth_modeled=(
      revenue_growth_model["Intercept"] + revenue_growth_model["gdp_growth"] * pl.col("gdp_growth")
    )
  )
  .with_columns(
    revenue_growth=pl.when(pl.col("type") == "Forecast")
      .then(pl.col("revenue_growth_modeled"))
      .otherwise(pl.col("revenue_growth"))
  )
)
```

The model estimates two parameters: (i) an intercept that captures the company’s baseline growth, and (ii) a slope coefficient that measures the company’s sensitivity to GDP changes. Then, we can use the model and the growth forecasts to make revenue growth projections. We visualize the historical and projected growth rates using this approach in [Figure 3](#fig-502):
...
revenue_growth = (dcf_data
  ...
  .unpivot(
    index=["year", "type"],
    on=["revenue_growth", "gdp_growth"],
    variable_name="name",
    value_name="value"
  )
)

revenue_growth = revenue_growth.with_columns(
  type=pl.col("type").cast(pl.Enum(["Realized", "Forecast"]))
)
...
[![Title: GDP growth and Microsoft's revenue growth and modeled forecasts between 2021 and 2030. The figure shows a line chart with years on the horizontal axis and GDP growth, revenue growth, and their projections on the vertical axis.](discounted-cash-flow-analysis_files/figure-html/dcf-fig-502-py-1.png)](discounted-cash-flow-analysis_files/figure-html/dcf-fig-502-py-1.png "Realized revenue growth rates are based on financial statements provided through the FMP API, while forecasts are modeled using IMF WEO forecasts.")

Realized revenue growth rates are based on financial statements provided through the FMP API, while forecasts are modeled using IMF WEO forecasts.
...
dcf_data = (dcf_data
  .sort("year")
  .with_columns(
    revenue_growth=pl.col("revenue_growth").fill_null(0)
  )
  .with_columns(
    revenue=pl.col("revenue").first() * (1 + pl.col("revenue_growth")).cum_prod()
  )
  .with_columns(
    ebit=pl.col("operating_margin") * pl.col("revenue"),
    depreciation_and_amortization=pl.col("da_margin") * pl.col("revenue"),
    taxes=pl.col("taxes_to_revenue") * pl.col("revenue"),
    delta_working_capital=pl.col("delta_working_capital_to_revenue") * pl.col("revenue"),
    capex=pl.col("capex_to_revenue") * pl.col("revenue"),
  )
  .with_columns(
    fcf=pl.col("ebit") + pl.col("depreciation_and_amortization") - pl.col("taxes") + pl.col("delta_working_capital") - pl.col("capex")
  )
...
We visualize the resulting FCF projections in [Figure 4](#fig-503).
...
[![Title: Actual and predicted free cash flow for Microsoft from 2021 to 2030. The figure shows a bar chart with years on the horizontal axis and Microsoft's historical and projected free cash flows on the vertical axis.](discounted-cash-flow-analysis_files/figure-html/fig-503-1.png)](discounted-cash-flow-analysis_files/figure-html/fig-503-1.png "Figure 4: Realized growth rates are based on financial statements provided through the FMP API, while forecasts are manually defined.")
...
[![Title: Actual and predicted free cash flow for Microsoft from 2021 to 2030. The figure shows a bar chart with years on the horizontal axis and Microsoft's historical and projected free cash flows on the vertical axis.](discounted-cash-flow-analysis_files/figure-html/dcf-fig-503-py-1.png)](discounted-cash-flow-analysis_files/figure-html/dcf-fig-503-py-1.png "Realized growth rates are based on financial statements provided through the FMP API, while forecasts are manually defined.")

Realized growth rates are based on financial statements provided through the FMP API, while forecasts are manually defined.
...
- The Capital Asset Pricing Model (CAPM) is a popular method to estimate the cost of equity \\r^E\\. It considers the risk-free rate, the equity risk premium, and the company’s risk exposure (i.e., beta). For a detailed guide estimating the CAPM, we refer to Chapter [Capital Asset Pricing Model](../chapters/capital-asset-pricing-model.llms.md).
...
wacc_raw = pl.read_excel(
  "wacc.xls", sheet_id=2, read_options={"header_row": 18}
)

import os
os.remove("wacc.xls")

wacc = wacc_raw.filter(
  pl.col("Industry Name") == "Computer Services"
)["Cost of Capital"][0]
```

You need the `fastexcel` package to use `pl.read_excel()`, so make sure to install it via, e.g., `pip install fastexcel`.
...
\\ \text{Total DCF Value} = \sum\_{t=1}^{T} \frac{\text{FCF}t}{(1 + \text{WACC})^t} + \frac{\text{TV}\_{T}}{(1 + \text{WACC})^T}, \\
...
sensitivity = pl.DataFrame(
  list(product(wacc_range, growth_rate_range)),
  schema=["wacc", "growth_rate"],
  orient="row"
)

sensitivity = sensitivity.with_columns(
  value=pl.struct(["wacc", "growth_rate"]).map_elements(
    lambda x: np.round(compute_dcf(x["wacc"], x["growth_rate"]) / 1e9, 0),
...
[![Title: Enterprise value of Microsoft for different WACC and growth rate scenarios. The figure shows a tile chart with different WACC values on the horizontal axis and perpetual growth rates on the vertical axis. Each tile shows a corresponding DCF value, illustrating the sensitivity of the DCF analysis to assumptions.](discounted-cash-flow-analysis_files/figure-html/dcf-fig-504-py-1.png)](discounted-cash-flow-analysis_files/figure-html/dcf-fig-504-py-1.png "The enterprise values combine data from FMP API, ad-hoc forecasts of financial ratios, and IMF WEO growth forecasts.")

The enterprise values combine data from FMP API, ad-hoc forecasts of financial ratios, and IMF WEO growth forecasts.

[Figure 5](#fig-504) reveals several key insights about our valuation: The valuation is highly sensitive to both WACC and growth assumptions, as small changes in either parameter can lead to substantial changes in enterprise value. Moreover, the relation between these parameters and company value is non-linear as the impact of growth rate changes becomes more pronounced at lower WACCs.
...
2.  Create a function that automatically generates FCF forecasts using different sets of ratio assumptions. Use it to create alternative scenarios for Microsoft.
3.  Implement an exit multiple approach for terminal value calculation and compare the results with the perpetuity growth method.
4.  Extend the sensitivity analysis to include operating margin assumptions. Create a visualization showing how changes in margins affect the final valuation.
5.  Calculate Microsoft’s equity value by adjusting the DCF value as described above.
