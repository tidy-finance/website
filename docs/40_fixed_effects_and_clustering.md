...
where $\varepsilon_t$ is i.i.d. normally distributed across time and firms. We use the `fixest::feols()`-function to estimate the simple model so that the output has the same structure as the other regressions below - you could also use `stats::lm()`. 
...
One way to tackle the issue of omitted variable bias is to get rid of as much unexplained variation as possible by including *fixed effects* - i.e., model parameters that are fixed for specific groups [@Wooldridge2010]. In essence, each group has its own mean in fixed effects regressions. The simplest group that we can form in the investment regression is the firm level. The firm fixed effects regression is then
