Recently, [Nikolaus Hautsch](https://homepage.univie.ac.at/nikolaus.hautsch/), [Albert J. Menkveld](https://albertjmenkveld.com/) and I released a new working paper [Market responses to a VIX shock](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=6298780) for which we analysed more than 30 billion order book events and trades in the S&P 500 index tracking ETF SPY. The paper is based on high-frequency data from [LOBSTER](https://lobsterdata.com/), which provides detailed information on the limit order book for all assets traded on NASDAQ.
...
The goal of [lobsteR](https://voigtstefan.github.io/lobsteR/) is to provide a tidy framework to request data from [LOBSTER](https://lobsterdata.com/), to download, unzip, and clean the data. The package focuses on the core functionalities required to get LOBSTER data ready fast.
...
[LOBSTER](https://lobsterdata.com/) reconstructs the complete history of limit order book snapshots from NASDAQ exchange data for all assets traded on the exchange, starting from June 27, 2006. For each trading day, [LOBSTER](https://lobsterdata.com/) delivers two files: a message file with nanosecond-timestamped records of every order book event, and an order book snapshot file capturing the state of the book after each event. This level of granularity offers unprecedented insights into the inner workings of some of the world’s most actively traded assets.

With `lobsteR` you can connect to [LOBSTER](https://lobsterdata.com/) using your own credentials.
...
I read the `.csv` files with `data.table::fread` rather than a tidyverse reader because `fread` is substantially faster for the large files LOBSTER produces. Two data issues need to be resolved upfront:
...
The cleaning pipeline above transforms raw LOBSTER files into a compact, analysis-ready dataset. In the [companion blog post](../../blog/hf-and-hugging-face/index.llms.md), we describe how we applied this pipeline to the full 2007–2025 LOBSTER history and made the resulting 5-second aggregated data for SPY freely available via `tidyfinance::download_data("tidyfinance", "high_frequency_sp500")`.
