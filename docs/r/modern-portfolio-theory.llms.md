...
> **NOTE:**
>
> You are reading **Tidy Finance with R**. You can find the equivalent chapter for the sibling **Tidy Finance with Python** [here](../python/modern-portfolio-theory.llms.md).
...
Thus, leveraging the approach introduced in [Working with Stock Returns](../r/working-with-stock-returns.llms.md), we download the constituents of the Dow Jones Industrial Average as an example portfolio as well as their daily adjusted close prices.
...
where \\\iota\\ is a vector of 1’s and \\\Sigma^{-1}\\ is the inverse of the variance-covariance matrix \\\Sigma\\. See [Proofs](../r/proofs.llms.md) in the Appendix for details on the derivation. In the following code chunk, we calculate the weights of the minimum-variance portfolio:
...
```
    ...
    # A tibble: 1 × 3
           mu  sigma type                      
        <dbl>  <dbl> <chr>                     
    1 0.00874 0.0324 Minimum-Variance Portfolio
...
where \\\lambda^\* = 2\frac{\bar\mu - D/C}{E-D^2/C}\\, \\C = \iota'\Sigma^{-1}\iota\\, \\D=\iota'\Sigma^{-1}\mu\\, and \\E=\mu'\Sigma^{-1}\mu\\. For details, we again refer to the [Proofs](../r/proofs.llms.md) in the Appendix.
...
[![Title: Efficient & minimum-variance portfolios. The figure shows big dots indicating the location of the minimum-variance and the efficient portfolio that delivers the expected return of the stock with the higehst return, respectively. The small dots indicate the location of the individual constituents.](modern-portfolio-theory_files/figure-html/fig-205-1.png)](modern-portfolio-theory_files/figure-html/fig-205-1.png "Figure 4: The big dots indicate the location of the minimum-variance and the efficient portfolio that delivers the expected return of the stock with the higehst return, respectively. The small dots indicate the location of the individual constituents.")
