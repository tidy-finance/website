...
Building on our analysis from the previous chapter on [Modern Portfolio Theory](../r/modern-portfolio-theory.llms.md), we again examine the Dow Jones Industrial Average constituents as an exemplary asset universe. We download and prepare our monthly return data:
...
\\\bar\mu = \tilde\mu^{\prime}\omega^\* = \frac{\lambda}{2}\tilde\mu^{\prime}\Sigma^{-1}\tilde\mu \\ \Rightarrow \lambda = \frac{2\bar\mu}{\tilde\mu^{\prime}\Sigma^{-1}\tilde\mu}.\\ Thus, the optimal portfolio weights are given by \\\omega^\* = \frac{\bar\mu}{\tilde\mu^{\prime}\Sigma^{-1}\tilde\mu}\Sigma^{-1}\tilde\mu.\\
...
Building on this framework, the Fama-French five-factor model ([Fama and French 2015](#ref-FamaFrench2015)) adds two more dimensions, which we later discuss in [Replicating Fama-French Factors](../r/replicating-fama-and-french-factors.llms.md):
