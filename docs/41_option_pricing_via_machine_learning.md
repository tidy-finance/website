...
Next, we split the data into a training set (which contains 1\% of all the observed option prices) and a test set which is only going to be used for the final evaluation. Note that the entire grid of possible combinations contains more than 3148992 different specifications, thus the sample to learn the Black-Scholes price contains only 3.148992\times 10^{4} and therefore is relatively small.
