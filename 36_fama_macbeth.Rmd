...
In their core, Fama-MacBeth regressions exploit a linear relationship between expected returns and exposure to (priced) risk factors. 
The basic idea of the regression approach is t project asset returns on factor exposures or characteristics that may resemble exposure to a risk factor in the cross section. 
Finally, the estimates are aggregated in the time dimension to test if a risk factor is priced. 
...
Generally, the Fama-MacBeth procedure is considered a two-step approach: 
The first step in the Fama-MacBeth procedure uses the exposures (characteristics) as explanatory variables in $T$ cross-sectional regressions, i.e.,
...
Here, we are interested in the compensation $\lambda^{f}_t$ for the exposure to each risk factor $\beta^{f}_{i,t}$ in each time point, i.e., the risk premium. Note the terminology: $\beta^{f}_{i,t}$ is a firm-specific characteristic. *If* there is a linear relationship between expected returns and the characteristic in a given month, we expect the regression coefficient to reflect the relationship, i.e., $\lambda_t^{f}\neq0$. 
...
It turns out that you can simply run a T-test on the time-series of $\hat\lambda^{f}_t$ to find the standard error of the mean. If you worry about serial correlation in the risk premiums, it is common to use the corrected standard errors of @Newey1987. 

The final remark before moving to the implementation concerns the coefficient estimates themselves. It is common, that the characteristics, e.g., $\hat\beta^{f}_{i}$ are estimated in a separate step. You might worry that the errors of the $\hat\beta^{f}_{i}$ estimated at the beginning impact the risk premium's standard errors.  Measurement error in $\hat\beta^{f}_{i}$ affects the risk premium estimates, i.e., they lead to biased estimates. The literature provides adjustments for this (see, e.g., @shanken1992, @kim1995, @chen2015, among others), but also shows that the bias goes to zero as $T \to \infty$. @gagliardini2016 provide an indepth discussion covering also the case of time-varying betas and estimation on individual stocks.
...
We think about the $\beta^{f}_{i}$ as an asset characteristic. In fact, we could also use asset characteristics like a firm's size or book-to-market ratio directly in the cross-sectional regression and skip the time-series regressions for such variables. 
...
Other authors use 24-month or 36-month windows for monthly returns or 240 trading days for data on daily frequency. 
...
characteristics <- data_famamacbeth %>%
  ...
  nest(data = -asset) %>% 
    mutate(lm = map(.x = data, 
                        ~broom::tidy(lm(ret_excess ~ . - month, 
                                        data = .x)))) %>% 
  unnest(lm) %>%
...
We regress the returns of the test assets at a particular point of time on the characteristics of each asset. 
...
Now that we have the risk premiums' estimates for each period, we can average across the time-series dimension to get the expected risk premium for each characteristic.

```{r}
risk_premiums %>% 
  group_by(term) %>% 
  summarise(risk_premium = mean(estimate), 
            t = mean(estimate) / sd(estimate) * sqrt(n()))
```
