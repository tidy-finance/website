...
While information on stocks and the underlying firms is readily provided by many news outlets, corporate bonds are not covered frequently. Additionally, the TRACE database contains trade-level information, which is also potentially new to students. Therefore, we provide you with some insights by showing some summary statistics.

We start by looking into the number of bonds outstanding over time. 

```{r, fig221}
expand_grid("date" = seq(as.Date("1990-01-01"), 
             as.Date("2018-12-01"), 
             by = "quarter"), 
  ...
  group_by(date) |> 
  summarize(outstanding_bonds = n(),
            .groups = "drop") |>
  ggplot(aes(x = date, y = outstanding_bonds)) +
  geom_line() +
  labs(
    x = NULL, y = NULL, color = NULL, linetype = NULL,
    title = "Number of bonds outstanding each quarter"
  )
```

Now that we know how many bonds are outstanding, you might also be interested in the question how many bonds are traded over time. We first also load our TRACE sample data into our session and then count the number of bonds traded each month.
...
trace_enhanced |> 
  ...
  group_by(month) |> 
  summarize(bonds_traded = length(unique(cusip_id)),
            .groups = "drop") |>
  ggplot(aes(x = month, y = bonds_traded)) +
  geom_line() +
  labs(
    x = NULL, y = NULL, color = NULL, linetype = NULL,
    title = "Number of bonds traded each month"
  )
```


Moreover, we also investigate the characteristics of corporate bonds. In particular, we consider the bonds maturity, coupon, and offering amount.

```{r}

```

Finally, let us compute some summary statistics for the trades that take place.
