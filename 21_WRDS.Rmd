...
We use the three remote tables to fetch the data we want to put into our local database. Just as above, the idea is that we let the WRDS database do all the work and just download the data that we actually need. We apply common filters and data selection criteria to narrow down our data of interest: (i) we keep only data in the time windows of interest, (ii) we keep only US-listed stocks as identified via share codes 10 and 11, and (iii) we keep only months with valid permno-specific information from `msenames`. In addition, we add delisting reasons and returns. You can read up in the great textbook of @BaliEngleMurray2016 for an extensive discussion on the filters we apply in the code below.
...
Similarly, we transform industry codes to industry descriptions following @BaliEngleMurray2016.\index{Industry codes} Notice that there are also other categorizations of industries (e.g., by Eugene Fama and Kenneth French) that are commonly used.
...
We also construct returns adjusted for delistings as described by @BaliEngleMurray2016. After this transformation, we can drop the delisting returns and codes.\index{Returns!Delisting}
...
```{r, fig211, fig.cap = "Monthly number of securities by listing exchange.", fig.alt = "Line chart of number of securities by exchange from 1960 to 2020 with years on the horizontal axis and the corresponding number on the vertical axis."}
crsp_monthly |>
  count(exchange, date) |>
...
```{r, fig215, fig.cap = "End-of-year share of securities with book equity values by listing exchange.", fig.alt = "Line chart of end-of-year shares of securities with book equity values by exchange from 1960 to 2020 with years on the horizongal axis and the coresponding share on the vertical axis."}
crsp_monthly |>
