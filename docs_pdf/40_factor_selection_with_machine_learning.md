...
To start, we restrict our analysis to just one industry: Energy. We first split the sample into a *training* and a *test* set. 
...
First, we focus on the predicted values $\hat{y}_t = x_t\hat\beta^\text{Lasso}.$ The figure below illustrates the projections for the *entire* time series of the Energy industry portfolio returns. The grey area indicates the out-of-sample period, which we did not use to fit the model.
