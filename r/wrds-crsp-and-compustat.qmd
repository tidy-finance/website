...
```{r}
crsp_monthly |> write_parquet("data-r/crsp_monthly.parquet")
```
...
crsp_daily_sub |> 
    group_by(batch = j) |> 
    write_dataset(path = "data-r/crsp_daily", format = "parquet", 
                  partitioning = "batch", existing_data_behavior = "overwrite") 
...
```{r}
compustat |> write_parquet("data-r/compustat.parquet")
```
