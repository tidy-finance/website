...
We use the two remote tables to fetch the data we want to put into our local database. Just as above, the idea is that we let the WRDS database do all the work and just download the data that we actually need. We apply common filters and data selection criteria to narrow down our data of interest: (i) we keep only data in the time windows of interest, (ii) we keep only US-listed stocks as identified via share type 
`"NS"`, , and (iii) we keep only months within permno-specific start dates `secinfostartdt` and end dates `secinfoenddt`.\index{Permno}

```{r}
# TODO: update text description above
# TODO: add column descriptions as comments to code below
crsp_monthly <- msf_db |>
...
```{r}
#| eval: false
# TODO: update column names to dsf_v2
