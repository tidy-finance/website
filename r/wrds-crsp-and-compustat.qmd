...
Another way to add `mktcap_lag` to the monthly CRSP data using the join approach from above is to use `lag_column()` from the `tidyfinance` package:

```{r}
#| eval: false
crsp_monthly <- crsp_monthly |> 
  group_by(permno) |> 
  mutate(mktcap_lag = lag_column(mktcap, date, months(1))) |> 
  ungroup()
```
...
The `tidyfinance` package provides shortcut to implement all these processing steps from above.

```{r}
#| eval: false
wrds_crsp_monthly <- download_data(
...
To fetch the linktable via `tidyfinance`, you can call:
