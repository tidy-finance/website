...
metadata:
  ...
  description-meta: Scale variations in choices of sample preparation to compute non-standard errors using the programming language R. 
---
...
1. The returns that we analyze above do not account for differences in the exposure to market risk, i.e., the CAPM beta. Change the function `compute_size_premium()` to output the CAPM alpha or beta instead of the average excess return. 
