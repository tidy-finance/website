...
The Capital Asset Pricing Model (CAPM) is one of the most influential theories in finance and builds on the foundation laid by [Modern Portfolio Theory](modern-portfolio-theory.qmd) (MPT). It was simultaneously developed by @Sharpe1964, @Lintner1965, and @Mossin1966. 
While MPT shows how to construct efficient portfolios, the CAPM extends this framework to explain how assets should be priced in equilibrium when all investors invest in efficient portfolios. The CAPM is the simplest model that aims to explain equilibrium asset prices and hence the cornerstone for a myriad of extensions. It is also used in industry applications, in particular for performance evaluation, as it provides a simple measure for market risk of an asset or portfolio. In this chapter, we derive the CAPM, discuss its empirical predictions and show how to estimate the core coefficients of the CAPM. We download stock market data, estimate betas using regression analysis, and evaluate asset performance.
...
Building on our analysis from the previous chapter on [Modern Portfolio Theory](modern-portfolio-theory.qmd), we again examine the Dow Jones Industrial Average constituents. We download and prepare our daily return data:
...
To recap, we considered a portfolio weight vector $\omega\in\mathbb{R}^N$ which invests into the available $N$ risky assets. Now, we will assume that the remainder, $1-\iota'\omega$, is invested in an available risk-free asset which pays $r_f$. The expected portfolio return is 
...
\Rightarrow \lambda = \frac{2\bar\mu}{\tilde\mu'\Sigma^{-1}\tilde\mu} \\
\Rightarrow \omega^* = \frac{\bar\mu}{\tilde\mu'\Sigma^{-1}\tilde\mu}\Sigma^{-1}\tilde\mu.$$

Note that $\omega^*$ does not sum up to one as, mechanically, the remainder $1-\iota'\omega^*$ is invested in the risk-free asset. 
Note, however, that scaling $\omega^*$ delivers the portfolio of risky assets such that $$\omega_\text{tan} := \frac{\omega^*}{\iota'\omega^*}= \frac{\Sigma^{-1}(\mu-r_f)}{\iota'\Sigma^{-1}(\mu-r_f)}.$$
Importantly, $\omega^\text{tan}$ is independent from $\bar\mu$! Thus, irrespective of the desired level of expected returns (or the investors risk aversion), everybody chooses the same portfolio of risky assets. The only variation arises from the amount of wealth invested into the risk-free asset. Some investors may even choose to lever their risky position by borrowing at the risk-free rate.
The portfolio $\omega_{tan}=\frac{\Sigma^{-1}\tilde\mu}{\iota'\Sigma^{-1}\tilde\mu}$ is typically called the efficient tangency portfolio.

Empirically, we take off from the visualization in the previous Chapter. To get some reasonably proxy for the risk free rate, we use the average 13-week T-bill rate (with symbol ^IRX). Since the prices are quoted in annualized percentage yields, we have to divide them by 100 and convert them to daily rates using 252 trading days.^[Note that the resulting returns have a 99% correlation with Fama-French risk free rate.] 
...
efficient_portfolios <- tribble(
  ~symbol,    ~mu,                                ~sigma,
  "omega^tan", 250 * t(w_tan) %*% mu,      sqrt(250 * t(w_tan) %*% sigma %*% w_tan),
  "r_f",       250 * rf,                           0)

sharpe_ratio <- sqrt(250) * t(w_tan) %*% (mu - rf) / sqrt(t(w_tan) %*% sigma %*% w_tan)
```
...
fig_capm <- assets |> 
  ...
  labs(x = "Volatility", y = "Average return",
       title = "Annualized returns and volatilities of Dow index constituents") +
  geom_abline(intercept = 250 * rf, 
              slope = sharpe_ratio,
              linetype = "dotted")
fig_capm
```
...
The slope of the line is, clearly, $\frac{\mu'\omega^* - r_f}{\omega^*\Sigma\omega^*}$. This slope is typically called the *Sharpe ratio*.

## The Capital Market Line

Taking a second look at the efficient tangency portfolio reveals that expected asset (excess) returns cannot be arbitrary large or small. From the first order condition of the optimization problem above we get via simple rearranging

$$\frac{\partial Z}{\partial \omega} = 2\Sigma\omega - \lambda (\mu-r_f) \overbrace{=}^{!}0 \\\Leftrightarrow \tilde\mu = \frac{2}{\lambda}\Sigma\omega^* \\
= \frac{2}{\lambda}\iota'\omega^*\Sigma\omega^\text{tan} \\ = \iota'\Sigma^{-1}\tilde\mu\Sigma\omega^\text{tan}$$

Now, note the following:
$$E(r^\text{tan}) = \omega^\text{tan}' \tilde\mu = 
\frac{\tilde\mu'\Sigma^{-1}\tilde\mu}{\iota'\Sigma^{-1}\tilde\mu}$$
and 
$$Var(r^\text{tan}) = \omega^\text{tan}' \Sigma \omega^\text{tan} = \frac{\tilde\mu'\Sigma^{-1}\tilde\mu}{\iota'\Sigma^{-1}\tilde\mu ^2}$$
It follows that $E(r^\text{tan}-r_f)/Var = \iota'\Sigma^{-1}\tilde\mu$ so we get that 
$$\tilde\mu_i = Cov(r_i,r^\text{tan})E()/Var$$
...
- In equilibrium, all investors hold some combination of the market portfolio and the risk-free asset, with their risk preferences determining only the proportion of each
