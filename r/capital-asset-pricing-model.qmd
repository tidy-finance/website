...
Asset returns vs systematic risk: the assets all fall onto the 45 degree line, as they should according to CAPM. 
...
fig_betas_returns <- assets |> 
  ...
  geom_point() +
  ggrepel::geom_label_repel(data = assets |> filter(symbol %in% c("BA", "NVDA")),
                            aes(label = symbol)) + 
  scale_y_continuous(labels = percent) + 
...
- Outperformance of small vs big companies (see [tidy-finance.org](https://www.tidy-finance.org/r/size-sorts-and-p-hacking.html))
- Outperformance of high vs low value companies (see [tidy-finance.org](https://www.tidy-finance.org/r/value-and-bivariate-sorts.html))
  
Fama-French 5-Factor model extends 3-factor model (see [tidy-finance.org](https://www.tidy-finance.org/r/replicating-fama-and-french-factors.html))
