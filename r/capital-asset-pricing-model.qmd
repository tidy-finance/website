...
Building on our analysis from the previous chapter on [Modern Portfolio Theory](modern-portfolio-theory.qmd), we again examine the Dow Jones Industrial Average constituents as an exemplary asset universe. We download and prepare our daily return data:
...
The relationship between risk and return is central to the CAPM. Often, it seems that higher volatility does not necessarily lead to higher expected returns. The CAPM's key insight states that not all risks are rewarded in equilibrium. Instead, only so-called systematic risk of an asset will determine the assets expected return. To understand this, we need to distinguish between systematic and idiosyncratic risk.

Company-specific events might affect individual stock prices, e.g., CEO resignations, product launches, and earnings reports. These idiosyncratic events don't necessarily impact the overall market and this asset-specific risk can be eliminated through diversification. Therefore, we call this risk idiosyncratic. Systematic risk, on the other hand, affects all assets in the market at the same time and investors really dislike it because it cannot be diversified away. TODO: Move this discussion to the CAPM equation?

## Portfolio Return & Variance

While we covered efficient portfolio choice in detail in the previous chapter, CAPM relies on the introduction of a crucial new element: a risk-free asset. This addition fundamentally changes the investment opportunity set, and leads to powerful conclusions about efficient portfolio choice.

To recap, we considered a portfolio weight vector $\omega\in\mathbb{R}^N$ which denotes investments into the available $N$ risky assets. So far we assumed that $\sum_i^N \omega_i=1$, indicating that all available wealth is invested across the asset universe without any outside option. Now, we relax this assumption and instead assume that all wealth remaining, $1-\iota'\omega$, is invested in an available risk-free asset which pays $r_f$. The expected portfolio return is 
$$\mu_\omega = \omega'\mu + (1-\iota'\omega)r_f = r_f + \omega'\underbrace{(\mu-r_f)}_{\tilde\mu}.$$
where $\mu$ is the the vector of expected return of assets and $r_f$ is the risk-free rate. In what follows, we often refer to $\tilde\mu$ as the vector of expected *excess* returns. 
...
where $\Sigma$ is the variance-covariance matrix of the returns. Restating the optimal decision problem of an investor who wants to earn a desired level of expected portfolio (excess) returns ($\bar\mu-r_f$) with the lowest possible variance leads to the Lagrangian
$$\min_\omega Z(\omega) = \min_\omega \omega'\Sigma\omega - \lambda \left(\omega'\tilde\mu-\bar\mu\right).$$
...
The constraint $\omega'\tilde\mu_\omega\geq \bar\mu$ delivers 
$$\bar\mu = \tilde\mu'\omega^* = \frac{\lambda}{2}\tilde\mu'\Sigma^{-1}\tilde\mu \\
\Rightarrow \lambda = \frac{2\bar\mu}{\tilde\mu'\Sigma^{-1}\tilde\mu}.$$ Thus, the optimal portfolio weights are given by 
$$\omega^* = \frac{\bar\mu}{\tilde\mu'\Sigma^{-1}\tilde\mu}\Sigma^{-1}\tilde\mu.$$

Note that $\omega^*$ does not necessarily sum up to one as, mechanically, the remainder $1-\iota'\omega^*$ is invested in the risk-free asset. 
Furthermore, note that scaling $\omega^*$ delivers the portfolio of risky assets $\omega_\text{tan}$ such that $$\omega_\text{tan} := \frac{\omega^*}{\iota'\omega^*}= \frac{\Sigma^{-1}(\mu-r_f)}{\iota'\Sigma^{-1}(\mu-r_f)}.$$
Importantly, $\omega^\text{tan}$ is independent from $\bar\mu$! Thus, irrespective of the desired level of expected returns (or the investors risk aversion), everybody chooses the same portfolio of risky assets. The only variation arises from the amount of wealth invested into the risk-free asset. Some investors may even choose to lever their risky position by borrowing at the risk-free rate and investing more than their actual wealth into the portfolio $\omega_\text{tan}$.
The portfolio $\omega_{tan}=\frac{\Sigma^{-1}\tilde\mu}{\iota'\Sigma^{-1}\tilde\mu}$ is central to the CAPM and is typically called the efficient tangency portfolio.

For illustrative purposes, we compute the efficient tangency portfolio for our asset universe. As a realistic proxy for the risk-free rate we use the 13-week T-bill rate (with the symbol ^IRX). Since the prices are quoted in annualized percentage yields, we have to divide them by 100 and convert them to daily rates using 252 trading days. 
...
rf <- risk_free_daily |> pull(risk_free) |> mean()
```
Next, we define the core parameters governing the distribution of asset returns, $\Sigma$ and $\mu$.
```{r}
assets <- returns_daily |> 
  group_by(symbol) |> 
  summarise(mu = 250 * mean(ret),
            sigma = sqrt(250 * var(ret)))
...
We are ready to illustrate the resulting efficient frontier. Every investor chooses to allocate a fraction of her wealth in the efficient tangency portfolio $\omega^\text{tan}$ and the remainder in the risk-free asset. The optimal allocation depends on the investor's risk aversion. As the risk-free asset, by definition, has a zero volatility, the efficient frontier is a straight line connecting the risk-free asset with the tangency portfolio. The slope of the line connecting the risk-free asset and the tangency portfolio is $\frac{\omega^\text{tan}'\mu-r_f}{\omega^\text{tan}'Sigma\omega^\text{tan}}$. Typically, the excess return of an asset scaled by its volatility, $\frac{\tilde\mu_i}{\sigma_i}$, is called the assets Sharpe ratio. Thus, the slope of the efficient frontier corresponds to the Sharpe ratio of the tangency portfolio returns. By construction, the tangency portfolio is the maximum Sharpe ratio portfolio.^[We proof in the Appendix that the efficient tangency portfolio $\omega^\text{tan}$ can also be derived as a solution to the optimization problem $\max_{\omega} \frac{\omega'\mu - r_f}{\omega'\Sigma\omega} \text{ s.t. } \omega'\iota=1.$]
...
mu_w <- t(w_tan) %*% mu
sigma_w <- sqrt(t(w_tan) %*% sigma %*% w_tan)

efficient_portfolios <- tribble(
  ~symbol,    ~mu,         ~sigma,
  "omega^tan", 250 * mu_w, sqrt(250) * sigma_w,
  "r[f]",       250 * rf,   0)

sharpe_ratio <- sqrt(250) * (mu_w - rf) / sigma_w
```

```{r}
#| label: fig-300
#| fig-cap: "The efficient frontier with a risk-free asset." 
#| fig-alt: "Title: Average returns and volatilities of Dow index constituents. The figure shows a scatter plot with volatities on the horizontal axis and average returns on the vertical axis."
...
fig_capm <- assets |> 
  ...
  title = "Annualized returns and volatilities") +
  geom_abline(aes(intercept = 250 * rf, slope = sharpe_ratio), linetype = "dotted")
...
Taking a second look at the efficient tangency portfolio $\omega^\text{tan}$ reveals that expected asset (excess) returns $\mu$ cannot be arbitrary large or small. From the first order condition of the optimization problem above we get, via simple rearranging,

$$\frac{\partial Z}{\partial \omega} = 2\Sigma\omega - \lambda (\mu-r_f)=0 \\\Leftrightarrow \tilde\mu = \frac{2}{\lambda}\Sigma\omega^* \\
= \frac{2}{\lambda}\iota'\omega^*\Sigma\omega^\text{tan} \\ = \iota'\Sigma^{-1}\tilde\mu\Sigma\omega^\text{tan}$$

Now, note the following:
$$E(\omega^\text{tan} 'r - r_f) = \omega^\text{tan} '\tilde\mu = 
\frac{\tilde\mu'\Sigma^{-1}\tilde\mu}{\iota'\Sigma^{-1}\tilde\mu}$$
and 
$$Var(\omega^\text{tan} 'r) = \omega^\text{tan} ' \Sigma \omega^\text{tan} = \frac{\tilde\mu'\Sigma^{-1}\tilde\mu}{(\iota'\Sigma^{-1}\tilde\mu)^2}$$
It follows that $E(\omega^\text{tan} 'r - r_f)/Var(\omega^\text{tan} 'r) = \iota'\Sigma^{-1}\tilde\mu$ so we get that 
$$\tilde\mu_i = Cov(r_i,r^\text{tan})E(\omega^\text{tan} 'r - r_f)/Var(\omega^\text{tan} 'r) = \beta_i E(\tilde\mu'\omega_\text{tan})$$
TODO: Interpretation. Assets expected return are proportional to covariance with the efficient portfolio excess returns. The price of risk is the excess return of the efficient tangency portfolio. An asset with 0 beta has the same expected return as the risk-free rate. An asset with a beta of 1 has the same expected return as the efficient tangency portfolio. An asset with a negative beta has expected returns lower than the risk-free asset - the very definition of an insurance. The CAPM equation is central to understand why some assets may have the same volatility but different returns: Because their systematic risk is different.
...
price_of_risk <- 250 * (t(w_tan) %*% mu - rf)

assets |> 
  ggplot(aes(x = beta, y = mu)) +
  geom_point() +
  geom_abline(intercept = 250 * rf, slope = price_of_risk) +
  scale_y_continuous(labels = percent) +
  labs(x = "Beta", y = "Average return",
       title = "Security market line") +
  annotate("text", x = 0, y = 250 * rf, label = "Risk-free", color = "blue")
```
...
Despite these stringent assumptions, the CAPM has become a cornerstone of modern finance. Its simplicity makes it an excellent foundation for more sophisticated models and a useful starting point for practical applications.
