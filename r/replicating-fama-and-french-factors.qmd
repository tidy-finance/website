...
We start with the three factor model. We already introduced the size and value factors in Chapter 9, and their definition remains the same. Size is the SMB factor (small-minus-big) that is long small firms and short large firms. The value factor is HML (high-minus-low) and is long in high book-to-market firms and short in low book-to-market counterparts. After the replication of the three factor model, we move to the five factors by constructing the profitability factor RMW (robust-minus-weak) as the difference between the returns of firms with high and low operating profitability and the investment factor CMA (conservative-minus-aggressive) as the difference between firms with high versus low investment rates. 
...
The results for the SMB factor are really convincing as all three criteria outlined above are met and the coefficient is around 0.99 and R-squared is at 99%. 
```{r}
summary(lm(smb ~ smb_replicated, data = test))
```

The replication of the HML factor is also a success, although at a slightly lower coefficient of 0.96 and R-squared around 96%. 
...
Now, let us move to the replication of the five factor model. We extend the `other_sorting_variables` data frame from above with the additional characteristics operating profitability `op` and investment `inv`. The re
...
In each month, we indepedently sort all stocks into the two size portfolios. The value, profitability, and investment, on the other hand, are the results of dependent sorts based on the size portfolios. We then merge the portfolios to the return data for the rest of the year just as above. 
...
Now, we want to construct each of the factors, but this time the size factor actually comes last because it is the result of averaging across all other factor portfolios. This depdency is the reason why we keep the table with value-weighted portfolio returns as a separate object that we reuse later. We construct the value factor as above by going long the two porfolios with high book-to-market ratios and shorting the two portfolios with low book-to-market.
...
For the profitability factor (i.e., RMW), we take a long position in the two high profitability portfolios and a short position in the two low profitability poertfolios.\index{Factor!Profitability}
...
For the investment factor (i.e., CMA), we go long the two low investment portfolios and short the two high investment portfolios.\index{Factor!Investment}
...
Finally, the size factor (i.e., SMB) is constructed by going long the six small portfolios and short the six large portfolios. 
...
The results for the SMB factor are quite convincing as all three criteria outlined above are met and the coefficient is around 0.97 and R-squared is at 99%. 
...
Overall, our approach seems to replicate the Fama-French three and five factor models quite well. We are curious to hear about any tweaks that could move the code above closer to the original factors.
