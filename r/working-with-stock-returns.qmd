...
## Annualization and Working with Different Frequencies

Financial data often exists at different frequencies due to varying reporting schedules, trading calendars, and economic data releases. For example, stock prices are typically recorded daily, while macroeconomic indicators such as GDP or inflation are reported monthly or quarterly. Additionally, some datasets are recorded only when transactions occur, resulting in irregular timestamps. To compare data meaningfully, we must align different frequencies appropriately. For example, to compare returns across different frequencies, we use annualization techniques.
...
```{r}
# Create monthly returns using base R functions instead of floor_date()
...
In this code, we first group the data by symbol and month and then compute monthly returns by compounding the daily returns: $(1+r_1)\cdots(1+r_2)\ldots(1+r_n)-1$. To visualize how return characteristics change across different frequencies, we can compare histograms:

```{r}
# Compare return distributions across frequencies for a single stock
...
# Plot histogram of returns by frequency
apple_returns |>
  ...
  labs(
    ...
    subtitle = "Note how the distribution changes with the time horizon"
  ) +
  scale_x_continuous(labels = percent) +
  theme_minimal() +
...
```{r}
# Compute annualized returns by symbol
...
# Define the mapping of frequency to the annualization factor
annual_factors <- c("Daily" = 252, "Monthly" = 12, "Yearly" = 1)

# Compute annual metrics dynamically for each frequency
annual_metrics <- apple_returns |>
  group_by(symbol, frequency) |>
  summarize(
    annual_factor = annual_factors[unique(frequency)],  # Assign appropriate AF
    arithmetic_annual_ret = mean(ret) * annual_factor,
    geometric_annual_ret = (prod(1 + ret)^(annual_factor / n()) - 1),
    annual_sd = sd(ret) * sqrt(annual_factor),
    sharpe_ratio = arithmetic_annual_ret / annual_sd,
    annual_var = quantile(ret, probs = 0.05) * sqrt(annual_factor),
    n_obs = n(),
    .groups = "drop"
  )
annual_metrics
