...
The following task can be easily parallelized to reduce computing time substantially. We use the parallelization capabilities of `mirai`. Note that we can also just recycle all the steps from above and collect them in a function.
...
selected_factors <- data |>
  nest(data = -industry) |>
  mutate(selected_variables = map(data, 
    in_parallel(\(data) select_variables(data), 
    select_variables=select_variables, lm_fit=lm_fit))
  )
