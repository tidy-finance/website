...
1. Estimate market betas based on daily CRSP using the `estimate_betas()` function from the `tidyfinance` package. Note that a memory-efficient way is to adjust the loop that we use above to handle daily CRSP data. 