...
In many instances, earning the lowest possible variance may not be the desired outcome. Instead, we generalize the concept of efficient portfolios, where, in addition to minimizing portfolio variance, the investor aims to earn a **minimum expected return** $\omega'\mu \geq \bar{\mu}.$ In other words, when $\bar\mu\geq \omega_\text{mvp}'\mu$, the investor is willing to accept a higher portfolio variance in return for earning a higher expected return. 
