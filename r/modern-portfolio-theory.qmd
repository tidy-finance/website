...
We introduce the @ggrepel package for adding text labels to the figures in this chapter.
...
We can use the tidy return data to quickly calcualte the estimated expected return of each asset in the Dow Jones Industrial Average as sample averages.
...
We can calculate the volatilities for each asset by simply using the `sd()` function.
...
The interpretation is straight-forward: a positive covariance indicates that assets move in the same direction, potentially increasing portfolio risk if these assets are combined; negative covariance indicates that assets move in opposite directions, which can reduce overall risk through diversification. 
...
Figure @fig-203 provides an illustration of the resulting variance-covariance matrix. 
...
where $\omega_i$ is the weight of asset $i$ in the portfolio and $\hat{\mu}_i$ is the estimated expected return of asset $i$. For simplicity, we assume that portfolio weights are contant over time for the current application. 

Portfolio variance is further calculated as
...
The optimization problem of the mean-variance approach aims to minimize this portfolio variance
...
Solving this problem analytically is easier in matrix notation, so we transform the problem: 

$$\min_{\omega} \omega' \hat{\Sigma} \omega \text{ s.t. } \omega'\iota = 1$$

The solution for the minimum-variance portfolio can then be described as
...
To illustrate the difference between the minimum-variance and efficient portfolio, we use the Nasdaq 100 index:
...
Figure @fig-205 now shows the average return and volatility of the the minimum-variance and efficient portfolio relative to the index constituents. We can see that the efficient portfolio dominates the minimum-variance portfolio in both dimensions. 
...
There are many ways to extend the Markowith framework. We want to point interested readers to the the `PortfolioAnalytics` package, which provides a consistent interface to many popular extensions. 

To illustrate the `PortfolioAnalytics` package, we replicate our analytic solutions from above. We additionaly use the `CVXR` package for a flexible suite of numerical optimizers.
...
Similarily, we can calculate the efficient portfolio by adding the return target constraint to the problem from above. Again, our 
...
The mean-variance framework is a cornerstone of modern finance, emphasizing the trade-off between risk and return. Using the `tidyfinance` package, it is straight-forward to download and implement this approach. We show how to compute minimum-variance and efficient portfolios using analyit solutions and via the `PortfolioAnalytics` package. The latter provides a robust interface for extending the mean-variance framework with additional constraints or alternative objectives. 
