...
According to @Markowitz1952, portfolio selection involves two stages: first, forming expectations about future security performance based on observations and experience; second, using these expectations to choose a portfolio. In this chapter, we focus on the latter and assume that there is no uncertainty in asset parameters.

Expected returns, denoted as $\mu_i$, represent the anticipated profit from holding an asset $i$ with $i=1,..., N$. For the sake of simplicity, we measure expected returns by computing the average of past returns:

$$\mu_i = \frac{1}{T} \sum_{t=1}^{T} r_{it},$$

where $r_{it}$ is the return of asset $i$ in period $t$, and $T$ is the total number of periods.
...
Figure @fig-201 shows the corresponding annualized average returns of the constituents of our example portfolio. 

```{r}
#| label: fig-201
#| fig-cap: "Annualized average returns based on prices adjusted for dividend payments and stock splits." 
#| fig-alt: "Title: Annualized average stock returns of Dow index constituents. The figure shows 30 bars with average daily returns."

annualize_return <- function(ret, trading_days = 252) {
  (1 + ret)^252 - 1
}

fig_mu <- assets |> 
  ggplot(aes(x = annualize_return(mu), y = fct_reorder(symbol, mu), fill = mu > 0)) +
  geom_col() +
...
Individual asset risk in MPT is typically quantified using variance ($\sigma^2$) or volatilities ($\sigma$).^[Alternative approaches include Value-at-Risk (VaR), Expected Shortfall, or higher-order moments such as skewness and kurtosis.] The latter can be calculated as the sample standard deviation:

$$\sigma_i = \sqrt{\frac{1}{T-1} \sum_{t=1}^{T} (r_{it} - \mu_i)^2}$$

We can get the volatilities for each asset by simply using the `sd()` function.
...
Figure @fig-202 shows the corresponding individual annualized stock volatilities. 

```{r}
#| label: fig-202
#| fig-cap: "Annualiezed volatilities based on prices adjusted for dividend payments and stock splits." 
#| fig-alt: "Title: Annualized volatilities of DOW index constituents. The figure shows 30 bars with daily volatilities."

annualize_volatility <- function(volatility, trading_days = 252) {
  volatility * sqrt(trading_days)
}

fig_sigma <- assets |> 
  ggplot(aes(x = annualize_volatility(sigma), y = fct_reorder(symbol, sigma))) +
  geom_col() +
  scale_x_continuous(labels = percent) + 
  labs(x = NULL, y = NULL,
       title = "Annualized volatilities of Dow index constituents")
fig_sigma
```

As highlighted above, a key innovation of MPT is to consider interactions between assets. *Covariance* provides a measure for these interactions and can be calculated as follows: \index{Covariance}
...
Since individual asset variances are just a special case of the covariance formula above with $i=j$, we can denote the variance-covariance matrix as:

$$\Sigma = \frac{1}{T-1}\sum\limits_{t=1}^T (r_t - \mu)(r_t - \mu)'.$$ 

To calculate this variance-covariance matrix, we can use the `cov()` function that takes a matrix of returns as inputs. We thus need to transform the returns from a tidy data frame into a $(T \times N)$ matrix with one column for each of the $N$ symbols and one row for each of the $T$ trading days. We achieve this by using `pivot_wider()` with the new column names from the `symbol`-column and setting the values to `ret`.
...
annualize_covariance <- function(covariance, trading_days = 252) {
...
fig_vcov <- vcov |> 
  ...
  ggplot(aes(x = symbol_a, y = fct_rev(symbol_b), fill = annualize_covariance(value))) +
  geom_tile() +
  labs(
...
$$\text{Expected Portfolio Return} = \sum_{i=1}^N \omega_i \mu_i,$$

where $\omega_i$ is the weight of asset $i$ in the portfolio and $\mu_i$ is the expected return of asset $i$. For simplicity, we assume that portfolio weights are constant over time. 

The portfolio variance is further calculated as

$$\text{Portfolio Variance} =\sum_{i=1}^{N} \sum_{j=1}^{N} \omega_i \omega_j \sigma_{ij},$$

where $\omega_i$ and  $\omega_j$ are the weights of assets $i$ and $j$ in the portfolio, respectively, and $\sigma}_{ij$ is the covariance between returns of assets $i$ and $j$.
...
$$\min_{\omega_1, ... \omega_n} \sum_{i=1}^{n} \sum_{j=1}^{n} \omega_i \omega_j \sigma_{ij},$$
...
$$\min_{\omega} \omega' \Sigma \omega \text{ s.t. } \omega'\iota = 1,$$

where $\Sigma$ is the $(N\times N)$ variance-covariance matrix and $\omega=\left(\omega_1,\ldots,\omega_N\right)'$ is the $(N\times1)$ vector of given portfolio weights. The solution for the minimum-variance portfolio can then be described as
...
Note that $\bar\mu$ needs to be higher than $\mu_{mvp}$.
...
Figure @fig-205 shows the average return and volatility of the minimum-variance and the efficient portfolio relative to the index constituents. We can see that the efficient portfolio has a higher annualized return at the cost of higher volatility compared to the minimum-variance portfolio. 
...
fig_summaries <- summaries |> 
  ggplot(aes(x = annualize_volatility(sigma), y = annualize_return(mu))) +
  geom_point(
