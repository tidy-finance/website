...
We can use the tidy return data to quickly calculate the estimated expected return of each asset in the Dow Jones Industrial Average as sample averages.
...
There are many ways to extend the Markowitz framework. We want to point interested readers to the the `PortfolioAnalytics` package, which provides a consistent interface to many popular extensions. 
