...
Thus, leveraging the approach introduced in [Working with Stock Returns](working-with-stock-returns.qmd), we download the constituents of the Dow Jones Industrial Average as an example portfolio, as well as their daily adjusted close prices. We compute the sample average returns as $\mu_i = \frac{1}{T} \sum_{t=1}^{T} r_{it},$ where $r_{it}$ is the return of asset $i$ in period $t$, and $T$ is the total number of periods and treat the vector $\mu = (\mu_1, ...,\mu_N)'$ as the true expected return of the assets.
...
We can use the tidy return data to quickly calculate the sample average of each asset in the Dow Jones Industrial Average that we can use as the expected return.
...
We can conveniently illustrate the resulting distribution of the asset returns in a diagram, showing the volatility on the x-axis and the expected return on the y-axis.

```{r}
#| label: fig-201
#| fig-cap: "Expected returns and volatilities based on prices adjusted for dividend payments and stock splits."
...
As highlighted above, a key innovation of MPT is to consider interactions between assets. The *Variance-Covariance matrix* collects this information. We suppose that the variance-covariance matrix $\Sigma$ of the returns is given by the sample estimator: \index{Covariance}
...
```{r}
#| label: fig-203
#| fig-cap: "Annualized variances and covariances based on prices adjusted for dividend payments and stock splits." 
...
Suppose now allocation your wealth in a portfolio given by the weight vector $\omega$. The resulting portfolio returns $\omega'r$ have an expected return $\mu_\omega = \omega'\mu = \sum_{i=1}^N \omega_i \mu_i$. 

The variance of the portfolio returns is $\sigma^2_\omega = \omega'\Sigma\omega = \sum_{i=1}^{N} \sum_{j=1}^{N} \omega_i \omega_j \sigma_{ij}$ where $\omega_i$ and  $\omega_j$ are the weights of assets $i$ and $j$ in the portfolio, respectively, and $\sigma}_{ij$ is the covariance between returns of assets $i$ and $j$.
...
while staying fully invested across all available assets $N$, $\sum_{i=1}^{N} \omega_i = 1$.
The analytical solution for the minimum-variance portfolio is
...
```{r}
mu_bar <- assets|>arrange(desc(mu))|>head(1) |> pull(mu)
```
...
Figure @fig-205 shows the average return and volatility of the minimum-variance and the efficient portfolio relative to the index constituents. As expected, that the efficient portfolio has a higher annualized return at the cost of higher volatility compared to the minimum-variance portfolio. 
...
fig_summaries <- summaries |> 
  ...
  labs(
    x = "Volatility", y = "Annualized return",
...
In other words, the efficient frontier can be characterized by the following equation:
...
fig_efficient_frontier <- summaries |> 
  ...
  scale_x_continuous(labels = percent) +
  scale_y_continuous(labels = percent) + 
  labs(x = "Volatility", y = "Annualized return",
