...
::: callout-note
You are reading **Tidy Finance with R**. You can find the equivalent chapter for the sibling **Tidy Finance with Python** [here](../python/setting-up-your-environment.qmd).
:::
...
If you plan to share your own code with collaborators or the public, you may encounter the problem that your projects requires sensitive information such as login credentials. Environment variables are widely used in software development projects because they provide a flexible and secure way to configure applications and store secrets. In later chapters, we use such environment variables to store private login data for a remote database. 

In R programming, you can use `.Renviron` files to store environment variables. Upon startup, R and RStudio look for `.Renviron` files in your home and project directory. `.Renviron` files can be at either at the user or project level. If there is a project-level `.Renviron`, the user-level file will not be sourced. A simple way, to create your own `.Renviron` file is 
```{r}
#| eval: false
usethis::edit_r_environ(scope = "project")
```
This command will open your `.Renviron` file and you can add variables. For the purpose of this book, we create the following variables (where `user` and `password` are our actual login credentials) and save the changes:
```
WRDS_USER=user
WRDS_PASSWORD=password
```
After you have restarted your RStudio session, you can access these environment variables via
```{r}
#| eval: false
Sys.getenv("WRDS_USER")
Sys.getenv("WRDS_PASSWORD")
```
Note that you can also store other login credentials, API keys, or file paths in the environment file.

If you use version control, then you should make sure that the `.Renviron` file is included in your `.gitignore`:
```{r}
#| eval: false
