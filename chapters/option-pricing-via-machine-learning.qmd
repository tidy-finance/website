...
$$
x_k^l = f\left(\theta_{k,0}^l + \sum\limits_{j = 1}^{N^{l-1}}z_j\theta_{k,j}^l\right)
$${#eq-neural-network}
...
$$
\begin{aligned}
  C(S, T) &= \Phi(d_1)S - \Phi(d_2)Ke^{-r_f T} \\
...
Next, we show how to fit a neural network to the data. The specification below defines a single layer feed-forward neural network with ten hidden units. Note that we fix the hyperparameters of all models in this chapter at sensible values to focus on the implementation workflow. In a real application, they should be tuned via cross-validation, as we illustrate in [Factor Selection via Machine Learning](factor-selection-via-machine-learning.qmd).
