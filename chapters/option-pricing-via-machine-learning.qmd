...
Here, $\theta$ are the parameters to fit, $N^{l-1}$ denotes the number of units in the previous layer (a hyperparameter to tune), and $z_j$ are the input variables, which can be either the raw data or, in the case of multiple chained layers, the outcome from a previous layer, $z_j = x_j^{l-1}$. While the easiest case with $f(x) = \alpha + \beta x$ resembles linear regression, typical activation functions are sigmoid (i.e., $f(x) = (1+e^{-x})^{-1}$) or ReLu (i.e., $f(x) = \max(x, 0)$).
...
Note that the entire grid of possible combinations contains `{r} formatC(nrow(option_prices), format = "d", big.mark = ",")` different specifications. Thus, the sample to learn the Black-Scholes price contains only `{r} formatC(round(nrow(option_prices) / 100), format = "d", big.mark = ",")` observations and is therefore relatively small.
