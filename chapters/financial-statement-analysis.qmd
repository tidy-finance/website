...
metadata:
  pagetitle: Financial Statement Analysis
  description-meta: Learn how to use R and Python to analyze financial statements using financial ratios.
cache: true
---
...
```{python}
#| label: financial-statement-analysis-packages-py
import polars as pl
...
```{python}
#| label: financial-statement-analysis-download-statements-py
sample = [
...
```{python}
#| label: financial-statement-analysis-liquidity-ratios-py
...
```{python}
#| label: financial-statement-analysis-leverage-ratios-py
...
@fig-411 provides a cross-sectional view of debt-to-asset ratios across our sample in 2023.
...
```{python}
#| label: financial-statement-analysis-efficiency-ratios-py
combined_statements = (balance_sheet_statements
  .select(
...
```{python}
#| label: financial-statement-analysis-profitability-ratios-py
...
@fig-414 illustrates the relationship between gross margins and profit margins across our sample of stocks in 2023.
...
#| fig-alt: "Title: Gross and profit margins for selected stocks in 2023. The figure shows a scatter plot with gross margins on the horizontal and profit margins on the vertical axis."
#| warning: false
combined_statements |>
  filter(fiscal_year == 2023) |>
...
@fig-415 compares Microsoft, Apple, and Amazon's rankings across four key financial ratio categories among our sample. Rankings closer to 1 indicate better performance within each category.
...
The Fama-French five-factor model aims to explain stock returns by incorporating specific financial metrics ratios. We provide more details in [Replicating Fama-French Factors](../r/replicating-fama-and-french-factors.qmd), but here is an intuitive overview:
...
```{python}
#| label: financial-statement-analysis-fama-french-factors-py
