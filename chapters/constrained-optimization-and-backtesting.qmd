...
aliases:
  ...
  - ../r/constrained-optimization-and-backtesting.html
  - ../python/constrained-optimization-and-backtesting.html
metadata:
  pagetitle: Constrained Optimization and Backtesting
  description-meta: Conduct portfolio backtesting with transaction costs and no-shortselling constraints using the programming languages R and Python.
---
...
```{python}
#| label: constrained-optimization-and-backtesting-packages-py
import polars as pl
import numpy as np
...
Compared to previous chapters, we introduce `expon` from `scipy.stats` to calculate exponential continuous random variables and `minimize` from `scipy.optimize` to perform numerical constrained optimization for portfolio choice problems.\index{Optimization}\index{Portfolio choice}
...
```{python}
#| label: constrained-optimization-and-backtesting-load-data-py
industry_returns = (
...
```{python}
#| label: constrained-optimization-and-backtesting-mvp-analytical-py
...
$${#eq-definition-transaction-costs}
...
```{python}
#| label: constrained-optimization-and-backtesting-efficient-weight-function-py
...
```{python}
#| label: constrained-optimization-and-backtesting-transaction-costs-py
gammas = [2, 4, 8, 20]
...
```{python}
#| label: constrained-optimization-and-backtesting-fig-1701-py
#| fig-cap: "The horizontal axis indicates the distance from the empirical minimum variance portfolio weight, measured by the sum of the absolute deviations of the chosen portfolio from the benchmark."
#| fig-alt: "Title: Portfolio weights for different risk aversion and transaction cost. The figure shows four lines that indicate that rebalancing from the initial portfolio decreases for higher transaction costs and for higher risk aversion."
rebalancing_figure = (
    ggplot(
...
We rely on the powerful `nloptr` package, which provides a common interface to a number of different optimization routines. In particular, we employ the Sequential Least-Squares Quadratic Programming (SLSQP) algorithm of @Kraft1994 because it is able to handle multiple equality and inequality constraints at the same time and is typically used for problems where the objective function and the constraints are twice continuously differentiable. We hence have to provide the algorithm with the objective function and its gradient, as well as the constraints and their Jacobian.
...
```{python}
#| label: constrained-optimization-and-backtesting-nloptr-replication-py
#| output: false
...
```{python}
#| label: constrained-optimization-and-backtesting-no-short-sale-py
...
```{python}
#| label: constrained-optimization-and-backtesting-regulation-t-py
reg_t = 1.5
...
```{python}
#| label: constrained-optimization-and-backtesting-fig-1702-py
#| fig-cap: "Optimal allocation weights for the 10 industry portfolios and the 4 different allocation strategies."
#| fig-alt: "Title: Optimal allocations for different strategies. The figure shows the portfolio weights for the four different strategies across the 10 different industries. The figures indicate extreme long and short positions for the efficient portfolio."
weights = (weights_mvp
...
```{python}
#| label: constrained-optimization-and-backtesting-l1-tc-function-py
...
```{python}
#| label: constrained-optimization-and-backtesting-backtest-setup-py
window_length = 120
...
```{python}
#| label: constrained-optimization-and-backtesting-helper-functions-py
...
```{python}
#| label: constrained-optimization-and-backtesting-rolling-window-py
...
```{python}
#| label: constrained-optimization-and-backtesting-performance-table-py
...
$$\omega_{t+1} ^* = \arg\max_{\omega \in \mathbb{R}^N, \iota'\omega = 1} \omega'\mu - \nu_t (\omega, B) - \frac{\gamma}{2}\omega'\Sigma\omega$${#eq-individual-tc-problem}
...
$$\nu_t\left(\omega, B\right) = \frac{\beta}{2} \left(\omega - \omega_{t^+}\right)'B\left(\omega - \omega_{t^+}\right)$${#eq-amihud-definition} where $B = \text{diag}(ill_1, \ldots, ill_N)$ is a diagonal matrix, where $ill_1, \ldots, ill_N$ measure the illiquidity of the individual assets. Derive a closed-form solution for the mean-variance efficient portfolio $\omega_{t+1} ^*$ based on the transaction cost specification above. Discuss the effect of illiquidity $ill_i$ on the individual portfolio weights relative to an investor that myopically ignores transaction costs in their decision.
