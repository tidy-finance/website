...
To speed the computation up, we parallelize the (many) different sorting procedures across the available cores. Finally, we report the resulting size premiums in descending order. There are indeed substantial size premiums possible in our data, in particular when we use equal-weighted portfolios.\index{Parallelization}
