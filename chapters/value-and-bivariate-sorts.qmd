...
The resulting monthly value premium is `{r} round(py$value_premium * 100, 2)` percent with an annualized return of `{r} round(((1 + py$value_premium)^12 - 1) * 100, 1)` percent.
...
The monthly value premium from dependent sorts is `{r} round(py$value_premium * 100, 2)` percent, which translates to an annualized premium of `{r} round(((1 + py$value_premium)^12 - 1) * 100, 1)` percent per year.
