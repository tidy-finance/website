...
aliases:
  ...
  - ../python/wrds-crsp-and-compustat.html
metadata:
  pagetitle: WRDS, CRSP, and Compustat
  description-meta: Use R and Python to download stock and firm data such as CRSP and Compustat from WRDS.
---
...
```{python}
#| label: wrds-crsp-and-compustat-packages-py
import polars as pl
import numpy as np
...
```{python}
#| label: wrds-crsp-and-compustat-date-range-py
...
WRDS is the most widely used source for asset and firm-specific financial data used in academic settings. WRDS is a data platform that provides data validation, flexible delivery options, and access to many different data sources. The data at WRDS is organized in an SQL database, although they use the [PostgreSQL](https://www.postgresql.org/) engine.\index{Database!PostgreSQL}

::: {.panel-tabset group="language"}
### R

This database engine is easy to handle with R. We use the `RPostgres` package to establish a connection to the WRDS database [@RPostgres]. Note that you could also use the `odbc` package to connect to a PostgreSQL database, but then you need to install the appropriate drivers yourself. `RPostgres` already contains a suitable driver.
...
This database engine is just as easy to handle with Python as SQL. We use the `sqlalchemy` package to establish a connection to the WRDS database because it already contains a suitable driver.^[An alternative to establish a connection to WRDS is to use the `WRDS-Py` library. We chose to work with `sqlalchemy` [@sqlalchemy] to show how to access PostgreSQL engines in general.]
...
To establish a connection to WRDS, you specify the WRDS server and your login credentials. We defined environment variables for the purpose of this book because we obviously do not want (and are not allowed) to share our credentials with the rest of the world.\index{Database!Connection}\index{Environment!Variables}
...
To establish a connection, you use the function `dbConnect()` with arguments that specify the WRDS server and your login credentials. See [Setting Up Your Environment](setting-up-your-environment.qmd) for information about why and how to create an `.Renviron`-file. Alternatively, you can replace `Sys.getenv("WRDS_USER")` and `Sys.getenv("WRDS_PASSWORD")` with your own credentials (but be careful not to share them with others or the public).
...
To establish a connection, you use the function `create_engine()` with a connection string that specifies the WRDS server and your login credentials. See [Setting Up Your Environment](setting-up-your-environment.qmd) for information about why and how to create an `.env`-file that can be loaded with `load_dotenv()`. Alternatively, you can replace `os.getenv("WRDS_USER")` and `os.getenv("WRDS_PASSWORD")` with your own credentials (but be careful not to share them with others or the public).

```{python}
#| label: wrds-crsp-and-compustat-connect-py
#| output: false
import os
...
```{python}
#| label: wrds-crsp-and-compustat-tidyfinance-connect-py
#| eval: false
...
```{python}
#| label: wrds-crsp-and-compustat-download-crsp-monthly-py
crsp_monthly_query = (
...
```{python}
#| label: wrds-crsp-and-compustat-add-mktcap-py
crsp_monthly = (crsp_monthly
...
```{python}
#| label: wrds-crsp-and-compustat-add-mktcap-lag-py
mktcap_lag = (crsp_monthly
    .with_columns(
...
```{python}
#| label: wrds-crsp-and-compustat-add-exchange-py
...
```{python}
#| label: wrds-crsp-and-compustat-add-industry-py
crsp_monthly = crsp_monthly.with_columns(
...
```{python}
#| label: wrds-crsp-and-compustat-add-excess-returns-py
...
```{python}
#| label: wrds-crsp-and-compustat-tidyfinance-crsp-monthly-py
#| eval: false
...
```{python}
#| label: wrds-crsp-and-compustat-drop-missing-py
crsp_monthly = (crsp_monthly
...
```{python}
#| label: wrds-crsp-and-compustat-write-crsp-monthly-py
#| output: false
...
```{python}
#| label: wrds-crsp-and-compustat-read-crsp-monthly-py
#| echo: false
...
#| fig-alt: "Title: Monthly number of securities by listing exchange. The figure shows a line chart with the number of securities by listing exchange from 1960 to 2024. In the earlier period, NYSE dominated as a listing exchange. There is a strong upwards trend for NASDAQ. Other listing exchanges do only play a minor role."
crsp_monthly |>
  count(exchange, date) |>
...
```{python}
#| label: wrds-crsp-and-compustat-fig-211-py
#| fig-cap: "Number of stocks in the CRSP sample listed at each of the US exchanges."
#| fig-alt: "Title: Monthly number of securities by listing exchange. The figure shows a line chart with the number of securities by listing exchange from 1960 to 2024. In the earlier period, NYSE dominated as a listing exchange. There is a strong upwards trend for NASDAQ. Other listing exchanges do only play a minor role."
...
```{r}
#| label: fig-212
#| fig-cap: "Market capitalization is measured in billion USD, adjusted for consumer price index changes such that the values on the horizontal axis reflect the buying power of billion USD in December 2024."
#| fig-alt: "Title: Monthly market cap by listing exchange in billion USD as of Dec 2024. The figure shows a line chart of the total market capitalization of all stocks aggregated by the listing exchange from 1960 to 2024, with years on the horizontal axis and the corresponding market capitalization on the vertical axis. Historically, NYSE listed stocks had the highest market capitalization. In the more recent past, the valuation of NASDAQ listed stocks exceeded that of NYSE listed stocks."
...
```{python}
#| label: wrds-crsp-and-compustat-fig-212-py
#| fig-cap: "Market capitalization is measured in billion USD, adjusted for consumer price index changes such that the values on the horizontal axis reflect the buying power of billion USD in December 2024."
#| fig-alt: "Title: Monthly market cap by listing exchange in billion USD as of Dec 2024. The figure shows a line chart of the total market capitalization of all stocks aggregated by the listing exchange from 1960 to 2024, with years on the horizontal axis and the corresponding market capitalization on the vertical axis. Historically, NYSE listed stocks had the highest market capitalization. In the more recent past, the valuation of NASDAQ listed stocks exceeded that of NYSE listed stocks."
...
#| fig-alt: "Title: Monthly number of securities by industry. The figure shows a line chart of the number of securities by industry from 1960 to 2024 with years on the horizontal axis and the corresponding number on the vertical axis. Except for stocks that are assigned to the industry public administration, the number of listed stocks decreased steadily at least since 1996. As of 2024, the segment of firms within public administration is the largest in terms of the number of listed stocks."
...
```{python}
#| label: wrds-crsp-and-compustat-fig-213-py
#| fig-cap: "Number of stocks in the CRSP sample associated with different industries."
#| fig-alt: "Title: Monthly number of securities by industry. The figure shows a line chart of the number of securities by industry from 1960 to 2024 with years on the horizontal axis and the corresponding number on the vertical axis. Except for stocks that are assigned to the industry public administration, the number of listed stocks decreased steadily at least since 1996. As of 2024, the segment of firms within public administration is the largest in terms of the number of listed stocks."
...
We also compute the market cap of all stocks belonging to the respective industries and show the evolution over time in @fig-214. All values are again in terms of billions of end of 2024 USD. At all points in time, manufacturing firms comprise of the largest portion of market capitalization. Toward the end of the sample, however, financial firms and services begin to make up a substantial portion of the market cap.
...
```{r}
#| label: fig-214
#| fig-cap: "Market capitalization is measured in billion USD, adjusted for consumer price index changes such that the values on the y-axis reflect the buying power of billion USD in December 2024."
...
```{python}
#| label: wrds-crsp-and-compustat-fig-214-py
#| fig-cap: "Market capitalization is measured in billion USD, adjusted for consumer price index changes such that the values on the y-axis reflect the buying power of billion USD in December 2024."
#| fig-alt: "Title: Monthly total market cap by industry in billions as of Dec 2024 USD. The figure shows a line chart of total market capitalization of all stocks in the CRSP sample aggregated by industry from 1960 to 2024 with years on the horizontal axis and the corresponding market capitalization on the vertical axis. Stocks in the manufacturing sector have always had the highest market valuation. The figure shows a general upwards trend during the most recent past."
...
```{python}
#| label: wrds-crsp-and-compustat-tidyfinance-crsp-daily-py
#| eval: false
...
```{python}
#| label: wrds-crsp-and-compustat-download-compustat-py
compustat_query = (
...
```{python}
#| label: wrds-crsp-and-compustat-add-be-op-py
compustat_annual = (compustat_annual
    .with_columns(
        be=(
...
```{python}
#| label: wrds-crsp-and-compustat-keep-last-firm-year-py
compustat_annual = (
...
```{python}
#| label: wrds-crsp-and-compustat-add-investment-py
...
```{python}
#| label: wrds-crsp-and-compustat-write-compustat-py
#| output: false
...
```{python}
#| label: wrds-crsp-and-compustat-tidyfinance-compustat-py
#| eval: false
...
```{python}
#| label: wrds-crsp-and-compustat-read-compustat-py
#| echo: false
compustat_annual = pl.read_parquet("data/compustat_annual.parquet")
```
...
```{python}
#| label: wrds-crsp-and-compustat-download-ccm-linking-table-py
...
```{python}
#| label: wrds-crsp-and-compustat-tidyfinance-ccm-links-py
#| eval: false
...
```{python}
#| label: wrds-crsp-and-compustat-create-ccm-links-py
ccm_links = (crsp_monthly
...
```{python}
#| label: wrds-crsp-and-compustat-merge-crsp-compustat-py
#| output: false
crsp_monthly.write_parquet("data/crsp_monthly.parquet")
```
...
```{r}
#| label: wrds-crsp-and-compustat-read-crsp-monthly-merged
#| echo: false
crsp_monthly <- read_parquet("data/crsp_monthly.parquet")
```

```{python}
#| label: wrds-crsp-and-compustat-read-crsp-monthly-merged-py
#| echo: false
crsp_monthly = pl.read_parquet("data/crsp_monthly.parquet")
```
...
```{python}
#| label: wrds-crsp-and-compustat-fig-215-py
#| fig-cap: "End-of-year share of securities with book equity values by listing exchange."
#| fig-alt: "Title: Share of securities with book equity values by exchange. The figure shows a line chart of end-of-year shares of securities with book equity values by exchange from 1960 to 2024 with years on the horizontal axis and the corresponding share on the vertical axis. After an initial period with lower coverage in the early 1960s, typically, more than 80 percent of the entries in the CRSP sample have information about book equity values from Compustat."
...
1. Compute `mktcap_lag` using `lag()` (R) or `shift()` (Python) rather than using joins as above. Filter out all the rows where the lag-based market capitalization measure is different from the one we computed above. Why are the two measures different?
1. Plot the average market capitalization of firms for each exchange and industry, respectively, over time. What do you find?
1. In the `compustat_annual` table, `datadate` refers to the date to which the fiscal year of a corresponding firm refers. Count the number of observations in Compustat by `month` of this date variable. What do you find? What does the finding suggest about pooling observations with the same fiscal year?
...
1. Before merging the CRSP and Compustat datasets, calculate and plot the proportion of Compustat firms that have corresponding stock return data in CRSP for each year. How does the coverage evolve over time?
</content>
</invoke>
