...
metadata:
  pagetitle: Replicating Fama-French Factors
  description-meta: Use the programming languages R and Python to replicate the famous Fama-French three- and five-factor asset pricing models.
---
...
```{python}
#| label: replicating-fama-and-french-factors-load-data-py
crsp_monthly = (
...
```{python}
#| label: replicating-fama-and-french-factors-sorting-variables-ff3-py
size = (crsp_monthly
...
```{python}
#| label: replicating-fama-and-french-factors-assign-portfolio-py
...
```{python}
#| label: replicating-fama-and-french-factors-merge-portfolios-ff3-py
portfolios = (crsp_monthly
    .with_columns(
...
```{python}
#| label: replicating-fama-and-french-factors-construct-factors-ff3-py
factors_replicated = (portfolios
    .group_by(["portfolio_size", "portfolio_bm", "date"])
...
In the previous section, we replicated the size and value premiums following the procedure outlined by Fama and French.\index{Size!Size premium}\index{Value premium} The final question is then: how close did we get? We answer this question by looking at the two time-series estimates in a regression analysis. If we did a good job, then we should see a non-significant intercept (rejecting the notion of systematic error), a coefficient close to one (indicating a high correlation), and an adjusted R-squared close to one (indicating a high proportion of explained variance).
...
```{python}
#| label: replicating-fama-and-french-factors-sorting-variables-ff5-py
other_sorting_variables = (compustat_annual
    .with_columns(
        sorting_date=pl.date(pl.col("datadate").dt.year() + 1, 7, 1)
    )
...
```{python}
#| label: replicating-fama-and-french-factors-portfolio-sorts-ff5-py
portfolios = (sorting_variables
    .group_by("sorting_date")
    .map_groups(lambda x: x
        .with_columns(
...
```{python}
#| label: replicating-fama-and-french-factors-value-factor-ff5-py
portfolios_value = (portfolios
    .group_by(["portfolio_size", "portfolio_bm", "date"])
    .agg(ret=(pl.col("ret_excess") * pl.col("mktcap_lag")).sum()
        / pl.col("mktcap_lag").sum())
...
```{python}
#| label: replicating-fama-and-french-factors-profitability-factor-ff5-py
portfolios_profitability = (portfolios
    .group_by(["portfolio_size", "portfolio_op", "date"])
    .agg(ret=(pl.col("ret_excess") * pl.col("mktcap_lag")).sum()
        / pl.col("mktcap_lag").sum())
...
```{python}
#| label: replicating-fama-and-french-factors-investment-factor-ff5-py
portfolios_investment = (portfolios
    .group_by(["portfolio_size", "portfolio_inv", "date"])
    .agg(ret=(pl.col("ret_excess") * pl.col("mktcap_lag")).sum()
        / pl.col("mktcap_lag").sum())
...
```{python}
#| label: replicating-fama-and-french-factors-size-factor-ff5-py
factors_size = (
    pl.concat(
...
```{python}
#| label: replicating-fama-and-french-factors-test-data-ff5-py
factors_replicated = (factors_size
