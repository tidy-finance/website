...
metadata:
  pagetitle: The Capital Asset Pricing Model
  description-meta: Learn how to use R and Python for estimating the CAPM for asset evaluation.
---
...
```{python}
#| label: capital-asset-pricing-model-packages-py
import polars as pl
import numpy as np
...
Building on our analysis from the previous chapter on [Modern Portfolio Theory](../r/modern-portfolio-theory.qmd), we again examine the Dow Jones Industrial Average constituents as an exemplary asset universe. We download and prepare our monthly return data:
...
```{python}
#| label: capital-asset-pricing-model-download-returns-py
...
```{python}
#| label: capital-asset-pricing-model-risk-free-py
risk_free_monthly = (
  pl.from_pandas(
...
```{python}
#| label: capital-asset-pricing-model-moments-py
assets = (returns_monthly
    .group_by("symbol")
    .agg(
        mu=pl.col("ret").mean(),
...
```{python}
#| label: capital-asset-pricing-model-tangency-portfolio-py
...
```{python}
#| label: capital-asset-pricing-model-security-market-line-py
...
```{python}
#| label: capital-asset-pricing-model-download-factors-py
...
```{r}
#| label: capital-asset-pricing-model-estimate-model-shortcut
...
- The SMB (Small Minus Big) factor captures the tendency of small stocks to outperform large stocks, as we discuss in our chapter [Size Sorts and P-Hacking](../r/size-sorts-and-p-hacking.qmd).
- The HML (High Minus Low) factor captures the tendency of value stocks to outperform growth stocks, as we show in [Value and Bivariate Sorts](../r/value-and-bivariate-sorts.qmd).

Building on this framework, the Fama-French five-factor model [@FamaFrench2015] adds two more dimensions, which we later discuss in [Replicating Fama-French Factors](../r/replicating-fama-and-french-factors.qmd):
