...
metadata:
  pagetitle: Parametric Portfolio Policies
  description-meta: Evaluate portfolio allocation strategies based on Brandt, Santa-Clara, and Valkanov (2009) using the programming languages R and Python.
---
...
```{python}
#| label: parametric-portfolio-policies-packages-py
import polars as pl
import pandas as pd
import numpy as np
...
To get started, we load the monthly CRSP file, which forms our investment universe. We load the data from our Parquet files introduced in [Accessing and Managing Financial Data](../r/accessing-and-managing-financial-data.qmd) and [WRDS, CRSP, and Compustat](../r/wrds-crsp-and-compustat.qmd).\index{Data!CRSP}
...
```{python}
#| label: parametric-portfolio-policies-load-crsp-py
crsp_monthly = (
...
```{python}
#| label: parametric-portfolio-policies-load-factors-py
factors_ff3_monthly = (
...
```{python}
#| label: parametric-portfolio-policies-compute-characteristics-py
crsp_monthly_lags = (crsp_monthly
...
```{python}
#| label: parametric-portfolio-policies-standardize-characteristics-py
...
```{python}
#| label: parametric-portfolio-policies-initial-theta-py
...
```{python}
#| label: parametric-portfolio-policies-compute-weights-function-py
...
```{python}
#| label: parametric-portfolio-policies-compute-weights-initial-py
weights_crsp = compute_portfolio_weights(
    theta,
    data_portfolios,
    value_weighting=True,
...
```{python}
#| label: parametric-portfolio-policies-power-utility-py
...
```{python}
#| label: parametric-portfolio-policies-evaluate-portfolio-function-py
...
```{python}
#| label: parametric-portfolio-policies-evaluate-initial-py
...
```{python}
#| label: parametric-portfolio-policies-objective-function-py
...
In its most basic form, Python optimization uses the function `minimize()`.\index{Optimization} As main inputs, the function requires an initial guess of the parameters and the objective function to minimize.

```{python}
#| label: parametric-portfolio-policies-optimize-theta-py
...
```{python}
#| label: parametric-portfolio-policies-evaluate-specifications-py
...
```{python}
#| label: parametric-portfolio-policies-performance-table-py
data = [data_portfolios]
...
```{python}
#| label: parametric-portfolio-policies-performance-optimal-py
performance_table.get(["EW Optimal", "VW Optimal"])
```

```{python}
#| label: parametric-portfolio-policies-performance-no-short-py
