...
treatment_date <- as.Date("2015-12-01")
...
```{r}
data_bond_raw <- trace_aggregated |> 
...
data_bond <- trace_aggregated |> 
  ...
  mutate(polluter = sic_code %in% polluting_industries,
         post_period = month > as.Date("2015-11-01"),
         treated = polluter & post_period,
         month = as.factor(month),
         month = relevel(month, ref = 15)) # TODO
  select(-sic_code)
```
...
```{r}
model_diff_months <- lm(
  formula = avg_yield ~ offering_amt + time_maturity + month:polluter,
  data = data_bond
)
