...
* [Stefan Voigt](https://www.voigtstefan.me/) is an Assistant Professor of Finance at the [Department of Economics at the University in Copenhagen](https://www.economics.ku.dk/) and a research fellow at the [Danish Finance Institute](https://danishfinanceinstitute.dk/). His research focuses on blockchain technology, market fragmentation, high-frequency trading, and financial econometrics. Stefan teaches parts of this book in his courses on empirical finance.
...
We met at the [Vienna Graduate School of Finance](https://www.vgsf.ac.at/), where we graduated with a different focus but a shared passion: coding with R. After years of painful trial and error, we decided to share our learnings with the public. 
...
Despite the vast number of empirical studies on financial phenomenons, one quickly learns that the actual implementation is typically rather opaque. 
As graduate students, we were particularly surprised by the lack of public code for seminal papers or even textbooks on key insights into financial economics. The lack of transparent codes not only leads to numerous replication efforts (and their failures) but is also a waste of resources on problems that countless others have already solved. 

This book aims to lift this curtain on reproducible finance by providing a fully transparent code base for many common financial applications. In addition, we hope to inspire others to share their code publicly and ensure the reproducibility of future findings. 
...
renv::dependencies(progress = FALSE) %>% 
  as_tibble() %>% count(Package, Version) %>% select(-n) %>% arrange(Package) %>% 
  kableExtra::kbl()
```
