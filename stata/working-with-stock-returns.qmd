...
At the start of each Stata script, we usually want to clear the memory and set more off ...
...
Stata unforunately has limited capabilities to download data directyl from online sources. We rather use the existing tidyfinance R package to download data and store it as DTA files.  
...
We are using temporary files for intermediate data because we don't want to pollute our project folder with unnecessary files. This is a result of Stata's key limitation of only beaing able to hold one data set in memory at the same time. 
