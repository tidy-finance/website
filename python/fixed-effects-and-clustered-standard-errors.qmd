...
Compared to previous chapters, we introduce `linearmodels` [@linearmodels], which provides tools for estimating various econometric models. 
...
compustat = pd.read_sql_query(
  sql="""SELECT datadate, gvkey, year, at, be, capx, oancf, 
                txdb 
          FROM compustat""",
  con=tidy_finance,
...
data_investment = (compustat
  .assign(
    month = lambda x: (pd.to_datetime(x["datadate"])
...
data_investment = (data_investment
  ...
  .assign(
    tobins_q = lambda x: ((x["mktcap"] + x["at"] - x["be"] + 
                            x["txdb"]) / x["at"])
  )
  .get(["gvkey", "year", "investment_lead", 
        "cash_flows", "tobins_q"])
  .dropna()
)
```
...
data_investment = (data_investment
  .assign(
    investment_lead = lambda x: 
      winsorize(x["investment_lead"], 0.01),
    cash_flows = lambda x: 
      winsorize(x["cash_flows"], 0.01),
    tobins_q = lambda x: 
      winsorize(x["tobins_q"], 0.01)
  )
)
```
...
np.round(data_investment_summary, 3)
```
...
model_fe_firm = lm.PanelOLS.from_formula(
  formula="""investment_lead ~ cash_flows + tobins_q + 
                               EntityEffects""",
  data=data_investment.set_index(["gvkey", "year"]),
).fit()
for table in model_fe_firm.summary.tables:
  print(table)
```
...
for table in model_fe_firmyear.summary.tables:
  print(table)
```
...
```{python}
comparison = lm.panel.results.compare(
  [model_ols, model_fe_firm, model_fe_firmyear]
)
for table in comparison.summary.tables:
  print(table)
```
...
Instead of relying on the iid assumption, we can use the `cov_type="clustered"` option in the `fit()`-function as above. The code chunk below applies both one-way clustering by firm as well as two-way clustering by firm and year.
...
```{python}
comparison_clustered = lm.panel.results.compare(
  [model_fe_firmyear, 
   model_cluster_firm, 
   model_cluster_firmyear]
)
for table in comparison_clustered.summary.tables:
  print(table)
```
