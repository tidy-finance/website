...
from sklearn.linear_model import ElasticNet, Lasso, lasso_path, enet_path
```
...
factors_ff_monthly = (
   pd.read_sql_query(sql="SELECT * FROM factors_ff_monthly",
                     con=tidy_finance,
...
factors_q_monthly = (
  pd.read_sql_query(sql="SELECT * FROM factors_q_monthly",
                    con=tidy_finance,
                    parse_dates={"month": {"unit": "D", "origin": "unix"}})
  .dropna()
...
macro_predictors = (
  pd.read_sql_query(sql="SELECT * FROM macro_predictors",
                    con=tidy_finance,
                    parse_dates={"month": {"unit": "D", "origin": "unix"}})
 .dropna()
 .add_prefix("macro_")
...
industries_ff_monthly = (
  pd.read_sql_query(sql="SELECT * FROM industries_ff_monthly",
                    con=tidy_finance,
                    parse_dates={"month": {"unit": "D", "origin": "unix"}})
  .dropna()
  .melt(id_vars="month", 
...
To illustrate penalized linear regressions, we employ the `scikit-learn` collection of packages for modeling and ML.

The `scikit-learn` workflow encompasses the main stages of the modeling process: pre-processing of data, model fitting, and post-processing of results. As we demonstrate below, `scikit-learn` provides efficient workflows that you can update with low effort.
...
For that purpose, `scikit-learn` provides the function `initial_time_split()` from the `rsample` package [@rsample]. 
...
data = data.assign(**dict(zip(column_names, new_column_values)))

preprocessor = ColumnTransformer(
    transformers=[
        ("scale", StandardScaler(), 
         [col for col in data.columns if col not in ["ret_excess", "month", "industry"]])
        ],
    remainder="drop",
...
```{python}
elastic_net_model = ElasticNet(alpha=0.0001, l1_ratio = 1.0, max_iter = 5000, fit_intercept=False)  
...
```{python}
# Only proceed with manufacturing data
data_manufacturing = data.query("industry == 'Manuf'")

# Apply pipeline to training data: 
pd.DataFrame(elastic_net_pipeline[:1].fit_transform(data_manufacturing.drop(columns = "ret_excess")), 
             columns = elastic_net_pipeline[:-1].get_feature_names_out())
             
lm_fit = elastic_net_pipeline.fit(data_manufacturing.drop(columns = "ret_excess"), 
...
predicted_values = (pd.DataFrame({
        "Fitted value": lm_fit.predict(data_manufacturing.drop(columns = "ret_excess")),
...
predicted_values_plot = (
  ggplot(predicted_values, 
         aes(x="month", y="return", color="Variable", linetype="Variable")) +
  geom_line() +
  labs(
    x="",
    y="",
...
```{python}
alphas_lasso, coefs_lasso, _ = lasso_path(preprocessor.fit_transform(data_manufacturing.drop(columns = "ret_excess")), 
                                         data_manufacturing.get("ret_excess"))

alphas_ridge, coefs_ridge, _ = enet_path(preprocessor.fit_transform(data_manufacturing.drop(columns = "ret_excess")), 
                                       data_manufacturing.get("ret_excess"),  
                                       l1_ratio=1e-2)
...
data_folds = TimeSeriesSplit(n_splits=n_splits, 
                             test_size=assessment_months, 
                             max_train_size=initial_years * length_of_year)

params = {
    "regressor__alpha":np.linspace(start = 0, stop = 1, num = 20),
...
finder = finder.fit(data_manufacturing.drop(columns="ret_excess"), 
                    data_manufacturing.get("ret_excess"))
```
...
validation_plot = (
  ggplot(pd.DataFrame(finder.cv_results_).assign(mspe = lambda x: np.sqrt(-x["mean_test_score"])), 
...
params = {
    "regressor__alpha":np.linspace(start = 0.001, stop = 1, num = 20),
}
...
for industry in all_industries:
  ...
  finder = finder.fit(data.query("industry==@industry"), 
                      data.query("industry==@industry").get("ret_excess"))
  results.append(pd.DataFrame(finder.best_estimator_.named_steps.regressor.coef_ != 0))

selected_factors  = (pd.DataFrame(lm_pipeline[:-1].get_feature_names_out(), 
                                  columns = ["variable"])
  .assign(variable = lambda x: x["variable"].str.replace("factor_|ff_|q_|macro_",""))
...
selected_factors_plot = (
  ggplot(selected_factors, aes(x="variable", y="industry", fill="value")) +
  geom_tile() +
