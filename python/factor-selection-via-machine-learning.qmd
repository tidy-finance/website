...
The training data is pre-processed according to our recipe steps, and the Lasso regression coefficients are computed. For illustrative purposes, we focus on the manufactoring industry for now.
...
alphas_lasso, coefs_lasso, _ = lasso_path(
  pre_processor.fit_transform(
    data_manufacturing
  ), 
  data_manufacturing.get("ret_excess")
)

alphas_ridge, coefs_ridge, _ = enet_path(
  pre_processor.fit_transform(
    data_manufacturing
  ), 
  data_manufacturing.get("ret_excess"),
...
validation = (pd.DataFrame(finder.cv_results_)
  .assign(
    mspe = lambda x: np.sqrt(-x["mean_test_score"]),
