...
characteristics = (compustat
  .assign(
    month = lambda x: (x["datadate"].dt.to_period("M")
                        .dt.to_timestamp())
  )
  ...
  .get(["gvkey", "bm", "log_mktcap", 
        "beta_monthly", "sorting_date"])
...
data_fama_macbeth = (data_fama_macbeth
  .merge(data_fama_macbeth_lagged, 
         how="left",
         on=["permno", "month"])
  .get(["permno", "month", "ret_excess_lead", 
        "beta", "log_mktcap", "bm"])
  .dropna()
)
```
...
risk_premiums = (data_fama_macbeth
  ...
  .apply(lambda x: smf.ols(
    "ret_excess_lead ~ beta + log_mktcap + bm", x
    ).fit().params
  )
)
```
