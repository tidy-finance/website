...
model_without_fe_time = (smf.ols(
    formula=("avg_yield ~ polluter + date_cat:polluter + time_to_maturity"
             " + log_offering_amt"),
    data=bonds_panel)
  .fit()
  .summary()
...
model_without_fe_coefs = (
  ...
  .query("term.str.contains('date_cat')")
  .assign(
    date=lambda x: 
...
model_with_fe_time = (lm.PanelOLS.from_formula(
    formula=formula,
    data=bonds_panel_alt.set_index(["cusip_id", "date"]))
  .fit()
  .summary
)
```
...
#| fig-pos: "htb"
polluter_plot = (
  ggplot(model_with_fe_time_coefs, 
         aes(x="date", y="estimate")) +
