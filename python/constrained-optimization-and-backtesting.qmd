...
w_reg_t_solution = w_reg_t_result.x
np.round(w_reg_t_solution, 3)
```
...
summary_df = performance_df.groupby("strategy").agg(
    ...
    Sharpe_ratio=("net_return", lambda x: (length_year * 100 * x.mean()) /
                  (np.sqrt(length_year) * 100 * x.std()) if x.mean() > 0 else np.nan),
