...
You can verify that `sum(w_no_short_sale$solution)` returns 1. In other words: `minimize()` provides the numerical solution to a portfolio choice problem for a mean-variance investor with risk aversion `gamma = 2`, where negative holdings are forbidden. 
