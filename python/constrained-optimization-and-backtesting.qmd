...
transaction_costs = (pd.DataFrame(list(product(gammas, betas)), 
                                  columns=["gamma", "beta"])
...
However, numerical optimization allows computing the solutions to such constrained problems. For the purpose of mean-variance optimization, we rely on the `minimize()` function from the module `scipy.optimize`.\index{Optimization!Constrained}

The function `minimize()` delivers numerical solutions to quadratic programming problems of an arbitrary form. For our purpose, we consider the form 
$$\min(-\mu \omega + 1/2 \omega' \Sigma \omega) \text{ s.t. } A' \omega >= b_0.$$
The function takes the argument `constraints` for equality and inequality constraints. Therefore, the above matrix $A$ is simply a vector of ones to ensure that the weights sum up to one. In the case of short-selling constraints, the matrix $A$ is of the form 
...
w_efficient_numerical = minimize(
  fun=lambda w: 2 * 0.5 * w.T @ Sigma @ w - (1 + mu) @ w,
  x0=w_initial,
  constraints=equality_constraints,
  tol=1e-20
)
...
w_reg_t = minimize(
  fun=lambda w: - w.T @ (1 + mu) + 2 * 0.5 * w.T @ Sigma @ w,
  x0=w_initial,
