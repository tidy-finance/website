...
::: {.callout-note}
You are reading **Tidy Finance with Python**. You can find the equivalent chapter for the sibling **Tidy Finance with R** [here](../r/option-pricing-via-machine-learning.qmd).
:::
...
$$R_1(j,s) = \{x \mid x_j < s\} \mbox{  and  } R_2(j,s) = \{x \mid x_j \geq s\}.$${#eq-decision-tree}
To pick $j$ and $s$, we find the pair that minimizes the residual sum of square (RSS):
$$\sum_{i:\, x_i \in R_1(j,s)} (y_i - \hat{y}_{R_1})^2 + \sum_{i:\, x_i \in R_2(j,s)} (y_i - \hat{y}_{R_2})^2$${#eq-decision-tree-split}
...
In finance, applications of neural networks can be found many different contexts, e.g. @Avramov2022, @Chen2019, and @Gu2020.
...
$$x_k^l = f\left(\theta^k_{0} + \sum\limits_{j = 1}^{N ^l}z_j\theta_{l,j}^k\right)$${#eq-neural-network}
...
One word of caution regarding the training of Neural networks: For illustrative purposes we sequentially update the parameters by reiterating through the data one thousand times (`max_iter=1000`). Typically, however, early stopping rules are adviced that aim to interrupt the process of updating parameters as soon the predictive performance on the validation test seems to deteriorate. A detailed discussion of these details in the implementation would go beyond the scope of this book. 
