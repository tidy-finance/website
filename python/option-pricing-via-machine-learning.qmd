...
```{python}
def black_scholes_price(S, K=70, r=0, T=1, sigma=0.2):
    d1 = (np.log(S / K) + (r + sigma**2 / 2) * T) / (sigma * np.sqrt(T))
    value = S * norm.cdf(d1) - K * np.exp(-r * T) * norm.cdf(d1 - sigma * np.sqrt(T))
    return value
```
...
# Create the Cartesian product of the input parameter values
option_prices = pd.DataFrame(list(product(S, K, r, T, sigma)), 
                             columns=["S", "K", "r", "T", "sigma"])

# TODO: assign black_scholes is embarassingly slow. Should we think about vectorization at this stage?
# TODO: right now we do not compute 2 random.normal innovations per price. 
np.random.seed(420)

option_prices = (option_prices
  .assign(black_scholes = lambda x: x.apply(lambda row: black_scholes_price(*row), axis=1),
...
```{python}
train_data, test_data = train_test_split(option_prices, 
...
```{python}
nnet_model = MLPRegressor(hidden_layer_sizes=(10, ), 
                          max_iter=500, 
                          random_state=42)
```
...
nnet_fit = nnet_pipeline.fit(train_data.drop(columns = ["observed_price"]), 
...
The random forests are trained using the function `RandomForestRegressor()`, which is required to be installed in order to run the code below. 
...
rf_fit = rf_pipeline.fit(train_data.drop(columns = ["observed_price"]), 
                         train_data.get("observed_price"))
```
...
```{python}
deepnnet_model = MLPRegressor(hidden_layer_sizes=(10, 10, 10), 
                              activation="logistic", 
                              solver="lbfgs", 
                              max_iter=500, 
                              random_state=42)
...
deepnnet_fit = deepnnet_pipeline.fit(train_data.drop(columns = ["observed_price"]), 
                                     train_data.get("observed_price"))
```
...
lm_fit = lm_pipeline.fit(train_data.get(["S", "K", "r", "T", "sigma"]), 
                         train_data.get("observed_price"))
```
...
predictive_performance = (pd.concat(
  ...
  .melt(id_vars=["S", "K", "r", "T", "sigma", "black_scholes", "observed_price"], 
        var_name="Model", 
...
predictive_performance_plot = (
  ggplot(predictive_performance, 
         aes(x="moneyness", y="pricing_error", color="Model", linetype="Model")) 
