...
This website is the online version of Tidy Finance with Python, a book soon available via Chapman & Hall/CRC. The book is the result of a joint effort of [Christoph Frey](https://sites.google.com/site/christophfrey/?utm_source=tidy-finance.org), [Christoph Scheuch](https://christophscheuch.github.io/?utm_source=tidy-finance.org), [Stefan Voigt](https://voigtstefan.me/?utm_source=tidy-finance.org), and [Patrick Weiss](https://sites.google.com/view/patrick-weiss?utm_source=tidy-finance.org).
...
Since the book's publication, we received great feedback from students and teachers alike. However, one of the most common feedback was that many interested coders are constrained and have to use Python in their institutions. We really love R for data analysis tasks, but we acknowledge the flexibility and popularity of Python. Hence, we decided to increase our team of authors with a Python expert and extend Tidy Finance to another programming language following the same tidy principles.

## Who Should Read This Book?

We write this book for three audiences:

- Students who want to acquire the basic tools required to conduct financial research ranging from undergraduate to graduate level. The book’s structure is simple enough such that the material is sufficient for self-study purposes.
...
- Good communication of results is a key ingredient to reproducible and transparent research. To compile this book, we heavily draw on a suite of fantastic open-source tools. First, @plotnine provide a highly customizable yet easy-to-use system for creating data visualizations based on the Grammar of Graphics [@Wilkinson2012]. Second, in our daily work and to compile this book, we used Quarto, an open-source scientific and technical publishing system described in @AllaireQuarto2022. Markdown documents are fully reproducible and support static and dynamic output formats. We do not provide introductions to these tools, as the resources above already provide easily accessible tutorials.\index{Quarto}
...
Python is prevalent for data science according to the Python Developers Survey [@pythonsurvey], particularly for financial applications. 
...
> Frey, C., Scheuch, C., Voigt, S., & Weiss, P. (2024).
...
@book{Frey2024,
  ...
  author = {Frey, Christoph and Scheuch, Christoph and Voigt, Stefan
            and Weiss, Patrick},
  year = {2024},
