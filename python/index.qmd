...
Python [@python] is a open-source, general-purpose, high-level programming language widely used across various industries.
It is the leading programming language for data science according to the Python Developers Survey [@pythonsurvey] and widely used in financial applications.
Python offers an object-oriented programming environment with a main focus on code readability and simplicity, making it an ideal choice for first time programmers and experienced financial researchers and analysts, allowing them to express complex ideas with concise and understandable code. Some of the highlights with Python include
- Open-source: Python uses open source license, making it freely usable and distributable for academic and also commercial use.
- Flexibility: Python's extensive ecosystem of standard library and the community-contributed modules allow for all kinds of amazing projects. It also seamless integration with various data sources and APIs, facilitating efficient data retrieval and processing. 
- Versatility: Python is a cross-platform, multipurpose language that can be used to write fast low-level executable code as well as large applications and even Graphical User Interfaces (GUI).
- Speed: Python is fast. While all code is translated at runtime to executable byte code, Python offers various dynamic compiling and parallilizaction techniques to speed code execution.
...
The so-called Zen of Python by Tim Peters summarizes its major synthax guidelines for structured, tidy and human readable code. It is easily accessable in every Python environment through: 
...
For more information on why Python is great, we refer to Chapter 1 in [@hilpisch2018].
