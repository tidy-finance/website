...
```{python}
fisd_query = (
  "SELECT complete_cusip, maturity, offering_amt, "
         "offering_date, dated_date, interest_frequency, "
         "coupon, last_interest_date, issue_id, issuer_id "
    ...
    "WHERE security_level = 'SEN' "
          ...
          "AND interest_frequency IN ('0', '1', '2', "
                                          "'4', '12') "
          "AND rule_144a = 'N' "
          "AND (private_placement = 'N' OR "
          ...
          "AND perpetual = 'N' "
          "AND (preferred_security = 'N' OR "
               "preferred_security IS NULL)"
)
...
fisd_issuer = (pd.read_sql_query(
    ...
    con=wrds,
    dtype={"issuer_id": int, "sic_code": str, 
...
We store code for cleaning enhanced TRACE with Python on the following Github [gist.](https://gist.github.com/patrick-weiss/86ddef6de978fbdfb22609a7840b5d8b) \index{Github!Gist}. The appendix also contains the code for reference. We only need to source the code from the gist, which we can do with with the code below using `httpimport`. In the chunk, we explicitly load the necessary function interpreting the Gist as a module (i.e., you could also use it as a module and preceed the function calls with the module's name). Alternatively, you can also go to the gist, download it, and manually execute it. The `clean_enhanced_trace()` function takes a vector of CUSIPs, a connection to WRDS explained in [WRDS, CRSP, and Compustat](wrds-crsp-and-compustat.qmd), and a start and end date, respectively. 
...
```{python}
fisd_cusips = list(fisd["complete_cusip"].unique())

batch_size = 100
batches = np.ceil(len(fisd_cusips) / batch_size).astype(int)
```
...
for j in range(1, batches + 1):
  ...
  cusip_chunk = fisd_cusips[
    ((j - 1) * batch_size):(min(j * batch_size, len(fisd_cusips)))
  ]

  cusip_str = "('" + "', '".join(cusip_chunk) + "')"
...
bonds_combined = pd.concat(
  [bonds_outstanding, bonds_traded], 
  ignore_index=True
)
...
average_characteristics = (fisd
  ...
  .melt(value_vars=["maturity", "coupon", "offering_amt"], 
        var_name="measure", 
        value_name="value")
  .dropna()
