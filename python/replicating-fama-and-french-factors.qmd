...
me_ff_dec = (data_ff
  ...
  .assign(
    sorting_date = lambda x: x["month"] + pd.DateOffset(months=7)
    )
...
def assign_portfolio(data, sorting_variable, percentiles):
    ...
    breakpoints.iloc[0] = min(data.get(sorting_variable))
    breakpoints.iloc[breakpoints.size-1] = max(data.get(sorting_variable))
