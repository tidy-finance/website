...
beta_portfolios = (data_for_sorts
  ...
  .apply(lambda x: x.assign(portfolio=pd.qcut(x["beta_lag"], q=[0, 0.5, 1], labels=["low", "high"]).astype("category")))
  .groupby(["portfolio", "month"], group_keys=False)
  .apply(lambda x: x.assign(ret=np.average(x["ret_excess"], weights=x["mktcap_lag"])))
)
```
...
model_fit = (sm.OLS
  ...
  .get_robustcov_results(cov_type="HAC", maxlags=6)
  .summary(slim=True)
)
model_fit
```
...
beta_portfolios_summary_figure = (ggplot(
  beta_portfolios_summary, aes(x="portfolio", y="alpha", fill="portfolio"))
  + geom_bar(stat="identity")
  + labs(title="CAPM alphas of beta-sorted portfolios",
         x="Portfolio", y="CAPM alpha", fill="Portfolio")
  + scale_y_continuous(labels=percent_format())
  + theme(legend_position="none"))
beta_portfolios_summary_figure.draw()
```
...
sml_capm_figure = (ggplot(beta_portfolios_summary,
...
beta_longshort_year_figure = (ggplot(
...
beta_longshort_year_figure.draw()
```
