...
Second, we add the column `symbol` that stores the ticker information, and finally, we rename all columns to lowercase names. Dataframes allow for *chaining* all these operations sequentially through using `.`.\index{Stock prices} \index{Chaining}
...
1. Download daily prices for another stock market symbol of your choice from Yahoo!Finance with `yf.download()` from the `yfinance` package. Plot two time series of the ticker’s unadjusted and adjusted closing prices. Explain the differences.
