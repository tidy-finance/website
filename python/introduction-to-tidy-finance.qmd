...
At the start of each session, we load the required packages. Throughout the entire book, we always use `pandas` and `numpy` to perform a number of data manipulations. In this chapter, we also load the convenient `yfinance` [@yfinance] package to download price data. 
