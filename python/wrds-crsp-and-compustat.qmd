...
@fig-211 shows the monthly number of securities by listing exchange over time. NYSE has the longest history in the data, but NASDAQ lists a considerably large number of stocks. The number of stocks listed on AMEX decreased steadily over the last couple of decades. By the end of `{python} crsp_monthly['date'].max().year`, there were `{python} crsp_monthly.filter((pl.col('date') == crsp_monthly['date'].max()) & (pl.col('exchange') == 'NASDAQ'))['permno'].n_unique()` stocks with a primary listing on NASDAQ, `{python} crsp_monthly.filter((pl.col('date') == crsp_monthly['date'].max()) & (pl.col('exchange') == 'NYSE'))['permno'].n_unique()` on NYSE, and `{python} crsp_monthly.filter((pl.col('date') == crsp_monthly['date'].max()) & (pl.col('exchange') == 'AMEX'))['permno'].n_unique()` on AMEX. \index{Exchange!NYSE}\index{Exchange!AMEX}\index{Exchange!NASDAQ}
...
Next, we look at the aggregate market capitalization grouped by the respective listing exchanges in @fig-212. To ensure that we look at meaningful data that is comparable over time, we adjust the nominal values for inflation. In fact, we can use the tables that are already in our folder to calculate aggregate market caps by listing exchange. All values in @fig-212 are in terms of the end of 2024 USD to ensure intertemporal comparability. NYSE-listed stocks have by far the largest market capitalization, followed by NASDAQ-listed stocks.\index{Data!CPI}

```{python}
#| label: fig-212
#| fig-cap: "The figure shows the monthly market capitalization by listing exchange. Market capitalization is measured in billion USD, adjusted for consumer price index changes such that the values on the horizontal axis reflect the buying power of billion USD in December 2024."
...
```{python}
#| label: fig-214 
#| fig-cap: "The figure shows the total Market capitalization in billion USD, adjusted for consumer price index changes such that the values on the y-axis reflect the buying power of billion USD in December 2024."
...
#| eval: false
compustat_annual = (
    compustat_annual.with_columns(year=pl.col("datadate").dt.year().cast(pl.Int64))
    .sort("datadate")
...
1. Go back to the original Compustat and extract rows where the same firm has multiple rows for the same fiscal year. What is the reason for these observations?
