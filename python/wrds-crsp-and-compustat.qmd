...
If you don't have access to WRDS, but still want to run the code in this book, we refer to the Appendix [WRDS Dummy Data](wrds-dummy-data.qmd), where show how to create a dummy database that contains the WRDS tables and corresponding columns such that all code chunks in this book can be executed with this dummy database.
...
for j in range(1, batches + 1):  
  ...
  crsp_daily_sub_query = (
    "SELECT permno, date, ret "
     ...
     f"WHERE permno IN {permno_string} " 
           f"AND date BETWEEN '01/01/1960' AND '12/31/2022'" 
  )
  ...
  if not crsp_daily_sub.empty:
      ...
      crsp_daily_sub = (crsp_daily_sub
        .merge(msedelist_sub[["permno", "dlstdt"]],
               on="permno", how="left")
        .assign(dlstdt=lambda x: x["dlstdt"].fillna("2022-12-31"))
        .query("date <= dlstdt")
        .drop(columns=["dlstdt"])
      )
...
compustat_query = (
  "SELECT gvkey, datadate, seq, ceq, at, lt, txditc, txdb, "
    ...
    "WHERE indfmt = 'INDL' "
          "AND datafmt = 'STD' "
          "AND consol = 'C' "
          "AND datadate BETWEEN '01/01/1960' AND '12/31/2022'"
)
