...
To get started, we load the monthly CRSP file, which forms our investment universe. We load the data from our database introduced in [Accessing and Managing Financial Data](accessing-and-managing-financial-data.qmd) and [WRDS, CRSP, and Compustat](wrds-crsp-and-compustat.qmd).\index{Data!CRSP}
...
```{python}
factors_ff_monthly = pd.read_parquet("data/factors_ff3_monthly.parquet")[["date", "mkt_excess"]]
```
