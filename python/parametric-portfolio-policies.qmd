...
def evaluate_portfolio(weights_data, 
    ...
    if capm_evaluation:
        ...
        evaluation_stats = pd.concat([evaluation_stats, 
                                      evaluation_capm], axis=1)
    
    if full_evaluation:
        weight_evaluation = (weights_data
            .melt(id_vars="month",
                  value_vars=["weight_benchmark", "weight_tilt"],
                  value_name="weight",
                  var_name="model")
             .groupby(["model", "month"])["weight"]
             .aggregate([("Mean Absolute weight", lambda x: np.mean(np.abs(x))),
                         ("Max. weight", max),
                         ("Min. weight", min),
              ...
              .reset_index()
              .groupby("model")
              .aggregate(lambda x: np.average(x) * 100) # TODO: Fix aggregation for month
              .reset_index()
              .assign(model = lambda x: x["model"].str.replace("weight_",""))
        )
        evaluation_stats = pd.concat([evaluation_stats, weight_evaluation.set_index("model")], axis=1)
        
    return evaluation_stats.transpose()
```
...
def objective_function(theta, 
    ...
    return -objective_function.loc[objective_measure, "tilt"]
```
...
```{python}
optimal_theta = minimize(fun=objective_function, 
                         x0=[1.5]*n_parameters,
...
def evaluate_optimal_performance(data, 
  ...
  processed_data = compute_portfolio_weights(optimal_theta, 
                                             data,
                                             value_weighting,
                                             allow_short_selling)
  ...
  portfolio_evaluation = (portfolio_evaluation
    .append(pd.Series({"benchmark": "VW" if value_weighting else "EW", 
                       "tilt": "Optimal (no s.)" if allow_short_selling else "Optimal"}, 
    index=portfolio_evaluation.columns, 
    name = "Value"))
  )
  
  return(portfolio_evaluation)

# Create the grid of parameter combinations
data = [data_portfolios]
value_weighting = [True, False]
allow_short_selling = [True, False]
objective_measure = ["Expected utility"]

permutations = itertools.product(data, value_weighting, allow_short_selling, objective_measure)
```
...
```{python}
results = list(itertools.starmap(evaluate_optimal_performance, permutations))
