...
data_portfolios = (crsp_monthly
  .assign(momentum_lag = lambda x: x["mktcap_lag"] / x["mktcap_13"],
...
data_portfolios = (data_portfolios
  ...
  .apply(lambda x: x.assign(relative_mktcap = x["mktcap_lag"] / x["mktcap_lag"].sum()))
  .set_index("month")
  .groupby(level="month")
...
def compute_portfolio_weights(theta, 
    ...
    data = (data
        .groupby("month", group_keys = True)
        .apply(lambda x: x.assign(characteristic_tilt = x.get(theta.index) @ theta / x.shape[0]))
        .reset_index(drop=True)
        .assign(weight_benchmark = lambda x: x["relative_mktcap"] if value_weighting else 1 / x.shape[0],
                weight_tilt = lambda x: x["weight_benchmark"] + x["characteristic_tilt"])
    ...
    if not allow_short_selling:
        data = (data
            ...
            .groupby("month", group_keys = True)
            .apply(lambda x: x.assign(weight_tilt = lambda x: x["weight_tilt"] / x["weight_tilt"].sum()))
            .reset_index(drop = True))
...
```{python}
def evaluate_portfolio(weights_crsp, 
    ...
    evaluation = (weights_crsp
      .groupby("month", group_keys = True)
      .apply(lambda x: pd.Series(np.average(x[["ret_excess", "ret_excess"]], 
    ...
    evaluation_stats = (evaluation
      ...
      .aggregate([("Expected utility", lambda x: np.mean(power_utility(x))),
                  ...
                  ("Sharpe ratio", lambda x: np.mean(x) / np.std(x) * np.sqrt(length_year))])
      .reset_index()
    )
    ...
    if full_evaluation:
        evaluation_weights = (weights_crsp
          ...
          .aggregate([("Absolute weight", lambda x: np.mean(np.abs(x))),
                      ("Max. weight", max),
                      ("Min. weight", min),
                      ("Avg. sum of negative weights", lambda x: -np.sum(x[x < 0])),
                      ("Avg. fraction of negative weights", lambda x: np.mean(x < 0))])
          .reset_index()
          .drop(columns="month")
          .groupby("model")
          .aggregate(lambda x: 100 * x.mean()) 
          .reset_index()
    ...
    evaluation_output = (evaluation_stats 
      .melt(id_vars=["model"], var_name="measure")
      .pivot_table(index="measure", columns='model', values='value')
      .reset_index()
    )
...
```{python}
test = evaluate_portfolio(weights_crsp)
test.loc[test["measure"]=="Expected utility", "tilt"]
```
...
def objective_function(theta, 
    ...
    processed_data = compute_portfolio_weights(theta, 
                                               data, 
                                               value_weighting, 
                                               allow_short_selling)
    ...
    objective_function = objective_function.loc[objective_function["measure"]==objective_measure, "tilt"]
...
def evaluate_optimal_performance(data, 
  ...
  processed_data = compute_portfolio_weights(optimal_theta.x, 
                                             data,
                                             value_weighting,
                                             allow_short_selling)
...
specifications = pd.DataFrame(
  product(
    ...
    [True, False], 
    [True, False]),
  columns=["data_portfolios_list", "objective_measure", 
           "value_weighting", "allow_short_selling"]
)

specifications["portfolio_evaluation"] = specifications.apply(
  lambda x: evaluate_optimal_performance(
    x["data_portfolios_list"],
    x["objective_measure"], 
    x["value_weighting"], 
    x["allow_short_selling"]
  ),
  axis=1
)
```
...
```{python}
# TODO: THIS UNNESTING DOES NOT WORK AAAAAHHHHH
performance_table = (specifications
  .get(["value_weighting", "allow_short_selling", "portfolio_evaluation"])
  .explode("portfolio_evaluation")
)

performance_table = (performance_table
  .rename(columns={"benchmark": " ", "tilt": "Optimal"})
  .assign(
    value_weighting=lambda x: x["value_weighting"].map({True: "VW", False: "EW"}),
    allow_short_selling=lambda x: x["allow_short_selling"].map({True: "", False: "(no s.)"})
  )
  .pivot(columns=["value_weighting", "allow_short_selling"], values=[" ", "Optimal"])
  .reset_index()
  .set_axis([f"{i[2]} {i[1]} {i[0]}" if i[0] in [" ", "Optimal"] else i[0] for i in performance_table.columns], axis=1, inplace=False)
)

selected_columns = ["measure", "EW    ", "VW    "] + sorted([col for col in performance_table.columns if "Optimal" in col])
performance_table = performance_table[selected_columns]
performance_table.head(11)
``` 
