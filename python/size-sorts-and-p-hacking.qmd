...
market_cap_concentration = (crsp_monthly
  .groupby("month")
  .apply(lambda x: x.assign(top01=(x["mktcap"] >= np.quantile(x["mktcap"], 0.99)).astype(int),
                            top05=(x["mktcap"] >= np.quantile(x["mktcap"], 0.95)).astype(int),
                            top10=(x["mktcap"] >= np.quantile(x["mktcap"], 0.90)).astype(int),
                            top25=(x["mktcap"] >= np.quantile(x["mktcap"], 0.75)).astype(int)))
  .reset_index(drop=True)
  .groupby("month")
  .apply(lambda x: pd.Series({"Largest 1% of stocks": (x["mktcap"][x["top01"] == 1].sum()) / x["mktcap"].sum(),
                              "Largest 5% of stocks": (x["mktcap"][x["top05"] == 1].sum()) / x["mktcap"].sum(),
                              "Largest 10% of stocks": (x["mktcap"][x["top10"] == 1].sum()) / x["mktcap"].sum(),
                              "Largest 25% of stocks": (x["mktcap"][x["top25"] == 1].sum()) / x["mktcap"].sum() }))
  .reset_index()
  .melt(id_vars="month", var_name="name", value_name="value")
)

market_cap_concentration_figure = (ggplot(
  market_cap_concentration, aes(x="month", y="value", color="name", linetype="name"))
  + geom_line()
...
market_cap_concentration_figure.draw()
```
...
market_cap_share_figure = (ggplot(
