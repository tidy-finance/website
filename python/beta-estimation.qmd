...
For the daily data, we use the same function as above but only take 3 months of data and require at least 50 daily return observations in these months. These restrictions help us to retrieve somewhat smooth coefficient estimates.
...
chunks = [chunk for _, chunk in crsp_daily.groupby('permno')]
        pool.map(calculate_beta, chunks)

    crsp_monthly_nested = calculate_beta(crsp_monthly_nested)
...
beta_daily = (crsp_daily
  .merge(valid_permnos, 
         ...
         on="permno")
  .groupby(["permno"], group_keys=False)
  .apply(lambda x: x.assign(beta_daily=roll_capm_estimation(x,
                                                            window_size,
                                                            min_obs)))
  .dropna()
)
