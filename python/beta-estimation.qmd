...
Compared to previous chapters, we introduce `statsmodels` for regression analysis and for sliding window functions, and `multiprocessing` to apply mapping functions in parallel.
...
model_fit.fit().summary(slim=True)
```
...
def roll_capm_estimation(data, window_size, min_obs):
    from statsmodels.regression.rolling import RollingOLS # parallelization in Windows requires standalone functions
    result = (RollingOLS.from_formula("ret_excess ~ mkt_excess",
                                      data=data,
                                      window=window_size,
                                      min_nobs=min_obs)
              .fit()
...
beta_example = (crsp_monthly
  ...
  .apply(lambda x: x.assign(beta=roll_capm_estimation(x,
                                                      window_size,
                                                      min_obs)))
  .dropna()
)
...
## CRSP sample Rolling-Window Estimation
...
beta_monthly = (crsp_monthly
  .merge(valid_permnos, 
         ...
         on="permno")
  .groupby(["permno"], group_keys=False)
  .apply(lambda x: x.assign(beta_monthly=roll_capm_estimation(x,
                                                              window_size,
                                                              min_obs)))
  .dropna()
)
...
betas = (crsp_daily
  .groupby(["permno"], group_keys=False)
  .parallel_apply(roll_capm_estimation, window_size, min_obs)
  .dropna()
)

beta_daily = crsp_daily.(assign = betas)
```
...
ggplot(beta_industries, aes(x="industry",y="beta_monthly"))+geom_boxplot()
plot_beta_industries = (ggplot(beta_monthly, aes(x="industry", y="beta_monthly"))
    + geom_boxplot()
    + coord_flip()
...
plot_beta_quantiles = (ggplot(beta_quantiles,
    aes(x='month', 
        y='beta_monthly', 
...
plot_beta_comparison = (ggplot(beta_comparison,
...
plot_beta_long = (ggplot(beta_long, 
