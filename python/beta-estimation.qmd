...
Compared to previous chapters, we introduce `statsmodels` for regression analysis and for sliding window regressions, and `pandarallel` to apply mapping functions in parallel.
...
def roll_capm_estimation(data, window_size, min_obs):
    # `RollingOLS` must be defined inside function for parallel_apply
    from statsmodels.regression.rolling import RollingOLS
...
beta_example = (crsp_monthly
  ...
  .apply(
    lambda x: x.assign(beta=roll_capm_estimation(x, window_size, min_obs))
  )
  .dropna()
)
```
...
Next, we can apply the code snippet from the example above to compute rolling window regression coefficients for all stocks. 
```{python}
beta_monthly = (crsp_monthly
  ...
  .groupby(["permno"], group_keys=False)
  .apply(
    lambda x: x.assign(beta_monthly=roll_capm_estimation(x, window_size, min_obs))
  )
  .dropna()
)
```
This is how to do it with the joblib package to use multiple cores:

```{python}
import joblib  # should go to the top
number_of_cores = 4

def roll_capm_estimation_for_joblib(permno, group):
    # First calculate rolling betas
    beta_values = (RollingOLS.from_formula(formula="ret_excess ~ mkt_excess",
                                           data=group,
                                           window=window_size,
                                           min_nobs=min_obs
                                           )
    ...
    # put all in Pandas DataFrame
    result_df = pd.DataFrame(beta_values)
    result_df.columns = ['beta']
    result_df['month'] = group['month'].values
    result_df['permno'] = permno
    result_df['ret_excess'] = group['ret_excess'].values
    result_df['mkt_excess'] = group['mkt_excess'].values
    try:  # to handle daily
        result_df['industry'] = group['industry'].values
        result_df['counts'] = group['counts'].values
    except(KeyError):
        pass
    result_df = result_df[list(result_df.columns[1:]) + [result_df.columns[0]]]
    return result_df


# Group permno data and calculate beta for each group
...
# Calculate beta values using joblib parallel processing
beta_monthly_joblib = (
    pd.concat(joblib.Parallel(n_jobs=number_of_cores)
...
# To check if all the betas are the same:
print((beta_monthly['beta_monthly'] - beta_monthly_joblib['beta']).sum())
...
crsp_daily = (pd.read_sql_query(
    sql="SELECT permno, month, date, ret_excess FROM crsp_daily",
    con=tidy_finance,
    parse_dates={"month": {"unit": "D","origin": "unix"},
...
factors_ff_daily = (pd.read_sql_query(
    sql="SELECT date, mkt_excess FROM factors_ff_daily",
    con=tidy_finance,
...
plot_beta_quantiles = (
  ggplot(beta_quantiles, 
         aes(x="month", y="beta_monthly", color="factor(quantile)")) +
  geom_line() +
