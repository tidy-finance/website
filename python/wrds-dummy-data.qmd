...
Let us initialize a `tidy_finance_python.sqlite` database or connect to your existing one. Be careful, if you already downloaded the data from WRDS, then the code in this blog post will overwrite your data!
...
time_series_years = np.arange(
  start_date.year, end_date.year + 1, 1
)
time_series_months = pd.date_range(
  start_date, end_date, freq="M"
)
time_series_days = pd.date_range(
...
industries = pd.DataFrame({
  "industry": ["Agriculture", "Construction", "Finance",
               ...
               "Wholesale"],
  "n": [81, 287, 4682, 8584, 1287, 1974, 1571, 4277, 1249, 
        457, 904],
...
stock_panel_yearly = pd.DataFrame({
  "gvkey": np.tile(stock_identifiers["gvkey"], 
                   len(time_series_years)),
  "year": np.repeat(time_series_years, 
                    len(stock_identifiers))
})

stock_panel_monthly = pd.DataFrame({
  "permno": np.tile(stock_identifiers["permno"], 
                    len(time_series_months)),
  "gvkey": np.tile(stock_identifiers["gvkey"], 
                   len(time_series_months)),
  "month": np.repeat(time_series_months, 
                     len(stock_identifiers)),
  "siccd": np.tile(stock_identifiers["siccd"], 
                   len(time_series_months)),
  "industry": np.tile(stock_identifiers["industry"], 
                      len(time_series_months)),
  "exchcd": np.tile(stock_identifiers["exchcd"], 
                    len(time_series_months)),
  "exchange": np.tile(stock_identifiers["exchange"], 
                      len(time_series_months))
})

stock_panel_daily = pd.DataFrame({
  "permno": np.tile(stock_identifiers["permno"], 
                    len(time_series_days)),
  "date": np.repeat(time_series_days, 
                    len(stock_identifiers))
})
```
...
beta_dummy = (stock_panel_monthly
  .assign(
    ...
    beta_daily = lambda x: (
      x["beta_monthly"] + 
        np.random.normal(scale=0.01, size=len(x))
    )
  )
)
...
commands = {
  col: np.random.rand(len(stock_panel_yearly)) 
...
compustat_dummy = (
  ...
  .assign(
    datadate = lambda x: 
      pd.to_datetime(x["year"].astype(str) + "-12-31")
  )
  .assign(**commands)
)
...
crsp_monthly_dummy = (stock_panel_monthly
  .assign(
    ...
    ret_excess = lambda x: (
      np.fmax(x["ret"] - 
        np.random.uniform(0, 0.0025, len(x)), -1)
    ),
    shrout = 1000 * np.random.uniform(
      1, 50, len(stock_panel_monthly)
    ),
    altprc = np.random.uniform(
      0, 1000, len(stock_panel_monthly))
    )
  .assign(
  ...
  .assign(
    mktcap_lag = lambda x: (
      x.groupby("permno")["mktcap"].shift(1)
    )
  )
...
crsp_daily_dummy = (stock_panel_daily
  .assign(
    ...
    ret_excess = lambda x: (
      np.fmax(np.random.normal(size=len(x)), -1)
    )
  )
  .reset_index(drop=True)
)
...
fisd_dummy = (pd.DataFrame({
    "complete_cusip": [
      generate_cusip() for _ in range(number_of_bonds)
    ]
  })
  .assign(
    maturity=lambda x: np.random.choice(
      time_series_days, len(x), replace=True
    ),
  ...
  .assign(
    offering_date=lambda x: x["maturity"] - pd.to_timedelta(
      np.random.choice(
        np.arange(1, 26) * 365, len(x), replace=True
      ), unit="D"
    )
  )
  .assign(
    dated_date=lambda x: x["offering_date"] - pd.to_timedelta(
      np.random.choice(
        np.arange(-10, 11), len(x), replace=True
      ), unit="D"
    ),
